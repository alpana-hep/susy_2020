...
TString legend_text[10] = {"prompt #gamma","non-prompt #gamma","genMatched Flag","Else","","","","",""};//{"No cuts","skimmed","lep-veto","isotrk-veto","Dphi-cut","MET>250","ST>300","Pho-pt>100"};
//TString legend_text[4] = {"(0#mu,0e) SR","(1#mu,0e) CR","pMSSM_MCMC_106_19786","pMSSM_MCMC_473_54451"};
int line_width[12] = {2,2,2,3,3,2,2,2,2,2,2,2};
int line_style[12] = {1,1,1,2,9,1,1,1,1,1,1,1};
// /int line_color[n_pl+1] = {kBlack, kRed, kGreen+2, kBlue, kRed};
int line_color[9] = {kMagenta+2, kGreen+2, kBlue, kBlack, kRed+3, kBlue + 2 , kCyan + 1 , kGreen + 3 };
...
void generate_1Dplot(vector<TH1D*> hist, char const *tag_name="", float energy=-1, int xrange=-1,int xmin=-1,char const *leg_head="",
...
c->SetLeftMargin(0.11);
  ...
  sprintf(lhead,"#bf{%s}:(1#gamma)",title);
  legend->SetHeader(lhead);
 legend->SetLineColor(kWhite);

  TLegendEntry* leg_entry[11];
    TLegendEntry* leg_entry1[11];
auto legend1 = new TLegend(0.7,0.6,0.9,0.72);
  ...
  for(int i = 0; i < (int)hist.size(); i++) {
    ...
    // }
    //    hist.at(i)=
    setLastBinAsOverFlow(hist.at(i));//,xrange);
    ...
    cout<<"range"<<"\t"<<xmin<<"\t"<<xrange<<endl;
    hist.at(i)= setMyRange(hist.at(i),xmin,xrange);
        hist.at(i)->GetXaxis()->SetRangeUser(xmin,xrange+1);
  ...
  for(int i = 0; i < (int)hist.size(); i++) {
    if(!normalize) hist.at(i)->GetYaxis()->SetRangeUser(0.001,ymax*20);
      ...
      {  hist.at(i)->GetYaxis()->SetRangeUser(0.00001,ymax*20.0);
...
void overlayPlots_v1(string pathname, bool which_Lept)
{
  ...
  if(which_Lept)
    {//Autumn18_lowPhotpT_WGJets_LL_estimation_Electron_wPho_pT_g30_MET100_phoID_loose.root
      f[0] = new TFile("Autumn18_GJets_DR-0p4_HT_v15v1_phoID_mva_wp90_MET100_pt40.root");//Autumn18_lowPhotpT_ZNuNuGJets_MonoPhoton_PtG_preUL_v15v1_phoID_mva_wp80_MET100_pt40.root");//Out_Autumn18_WGJets_LL_estimation_v18_Muon_wPho_pT_g40_MET100.root");
      //Autumn18_lowPhotpT_WJetsToLNu_HT_LL_estimation_Electron_wPho_pT_g40_MET100_phoID_loose.root
      f[1]=new  TFile("Autumn18_QCD_HT_v15v1_phoID_mva_wp90_MET100_pt40.root");//Autumn18_lowPhotpT_ZNuNuJets_ZNuNuGJets_preUL_v15v1_phoID_mva_wp80_MET100_pt40.root");//Autumn18_lowPhotpT_ZNuNuJets_HT_preUL_v15v1_phoID_mva_wp80_MET100_pt40.root");//Out_Autumn18_Combined_WGJets_WJets_LL_estimation_v18_Muon_wPho_pT_g40_MET100.root");//Out_Autumn18_WJets_LL_estimation_v18_Muon_wPho_pT_g40_MET100.root");//Autumn18_lowPhotpT_WJetsToLNu_HT_LL_estimation_Electron_wPho_pT_g40_MET100_phoID_mva_wp90.root");////Autumn18_lowPhotpT_WGJetsplusWJets_Stitched_LL_estimation_Electron_wPho_pT_g40_MET100_phoID_tight.root");
...
else
    {
       ...
       f[1]=new  TFile("Out_Autumn18_Combined_WGJets_WJets_LL_estimation_v18_Muon_wPho_pT_g40_MET100.root");//Out_Autumn18_WJets_LL_estimation_v18_Muon_wPho_pT_g40_MET100.root");
  ...
  for(int i_file=0; i_file<1;i_file++)
    {
      sprintf(hname,"EventYields_prompt_nonprompt_Photon_LL_allProcess_lep_%d_%s_loose.txt",which_Lept,filetag[i_file]);
      std::ofstream file_;
      file_.open(hname,ios::out);
      vector<TH1D*> hist_list_Njets;
      vector<TH1D*> hist_list_Bjets;
      ...
      vector<TH1D*> hist_list_ST;
      vector<TH1D*> hist_list_HT;
      vector<TH1D*> hist_BDTresp;
      for(int i_cut=0; i_cut<4;i_cut++)
	{
	  //if(i_cut>0 && i_cut!=6)continue;
	  //	  if(i_file<6 )
	  cout<<baseline1[i_cut].c_str()<<endl;
	     sprintf(hist_name,"h_NhadJets_v1_%s",baseline1[i_cut].c_str());
	      sprintf(hist_name1,"h_NBJets_v1_%s",baseline1[i_cut].c_str());
	      sprintf(hist_name2,"h_MET_v1_%s",baseline1[i_cut].c_str());
	      sprintf(hist_name3,"h_PhoPt_v1_%s",baseline1[i_cut].c_str());
	      //sprintf(hist_name4,"h_Mt_phoMET_%s",baseline1[i_cut].c_str());
	      sprintf(hist_name5,"h_HT_v1_%s",baseline1[i_cut].c_str());
	      sprintf(hist_name6,"h_St_v1_%s",baseline1[i_cut].c_str());
	      sprintf(hist_name7,"h_BDT_response_v1_%s",baseline1[i_cut].c_str());
	  ...
	  TH1D* hist_Njets_temp = (TH1D*)f[i_file]->Get(hist_name);
	  TH1D* hist_Bjets_temp = (TH1D*)f[i_file]->Get(hist_name1);
	  TH1D* hist_MET_temp = (TH1D*)f[i_file]->Get(hist_name2);
	  TH1D* hist_phopt_temp = (TH1D*)f[i_file]->Get(hist_name3);
	  //	  TH1D* hist_Mt_temp = (TH1D*)f[i_file]->Get(hist_name4);
	  // TH1D* hist_Ht_temp = (TH1D*)f[i_file]->Get(hist_name5);
	  TH1D* hist_ST_temp = (TH1D*)f[i_file]->Get(hist_name6);
	  // TH1D* hist_BDTresp_temp = (TH1D*)f[i_file]->Get(hist_name7);
	  ...
	  hist_ST_temp->GetXaxis()->SetRangeUser(300,2500);
	  ...
	  //hist_phopt_temp->GetXaxis()->SetRangeUser(0,600);
	  //  setLastBinAsOverFlow(hist_Bjets_temp,12);
          // setLastBinAsOverFlow(hist_Njets_temp,16);
          // setLastBinAsOverFlow(hist_MET_temp,1500);
          // setLastBinAsOverFlow(hist_phopt_temp,1000);
          // setLastBinAsOverFlow(hist_Ht_temp,3000);
          // setLastBinAsOverFlow(hist_ST_temp,3000);
	  // TH1D* hist_ST_temp1 =  DrawOverflow(hist_ST_temp);
      ...
      int xrange=0.0;
      file_<<filetag[i_file]<<"\t"<<hist_list_Njets.at(0)->Integral()<<"\t"<<hist_list_Njets.at(1)->Integral()<<"\t"<<hist_list_Njets.at(2)->Integral()<<"\t"<<hist_list_Njets.at(3)->Integral()<<endl;
      ...
      sprintf(full_path,"%s/wPho_pt40_NJets_%s_",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_Njets,full_path,energy,16,0,leg_head,false,true,false,true,filetag[i_file]);
      sprintf(full_path1,"%s/wPho_pt40_N-BJets_%s_",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_Bjets,full_path1,energy,10,0,leg_head,false,true,false,true,filetag[i_file]);
      sprintf(full_path2,"%s/wPho_pt40_MET_%s_",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_MET,full_path2,energy,1500,0,leg_head,false,true,false,true,filetag[i_file]);
      sprintf(full_path3,"%s/wPho_pt40_PhoPt_%s_",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_PhoPt,full_path3,energy,600,0,leg_head,false,true,false,true,filetag[i_file]);
      // sprintf(full_path4,"%s/wPho_pt40_Ht_%s_",pathname.c_str(),filetag[i_file]);
      // generate_1Dplot(hist_list_HT,full_path4,energy,3000,0,leg_head,false,true,false,true,filetag[i_file]);
      sprintf(full_path5,"%s/wPho_pt40_ST_%s_",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_ST,full_path5,energy,3000,0,leg_head,false,true,false,true,filetag[i_file]);
      // sprintf(full_path5,"%s/wPho_pt40_BDT_resp_%s_",pathname.c_str(),filetag[i_file]);
      // generate_1Dplot(hist_BDTresp,full_path5,energy,1,-1,leg_head,false,true,true,true,filetag[i_file]);


      sprintf(full_path,"%s/wPho_pt40_NJets_%s_normalized",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_Njets,full_path,energy,16,0,leg_head,true,true,false,true,filetag[i_file]);
      sprintf(full_path1,"%s/wPho_pt40_N-BJets_%s_normalized",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_Bjets,full_path1,energy,10,0,leg_head,true,true,false,true,filetag[i_file]);
      sprintf(full_path2,"%s/wPho_pt40_MET_%s_normalized",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_MET,full_path2,energy,1500,0,leg_head,true,true,false,true,filetag[i_file]);
      sprintf(full_path3,"%s/wPho_pt40_PhoPt_%s_normalized",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_PhoPt,full_path3,energy,600,0,leg_head,true,true,false,true,filetag[i_file]);
      // sprintf(full_path4,"%s/wPho_pt40_Ht_%s_normalized",pathname.c_str(),filetag[i_file]);
      // generate_1Dplot(hist_list_HT,full_path4,energy,3000,0,leg_head,true,true,false,true,filetag[i_file]);
      sprintf(full_path5,"%s/wPho_pt40_ST_%s_normalized",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_ST,full_path5,energy,3000,0,leg_head,true,true,false,true,filetag[i_file]);
      // sprintf(full_path5,"%s/wPho_pt40_BDT_resp_%s_normalized",pathname.c_str(),filetag[i_file]);
