...
// /int line_color[n_pl+1] = {kBlack, kRed, kRed+2, kBlue, kRed};                                                                               
int line_color[9] = {kRed,kRed,kRed+2,kViolet+2,kRed-2,kYellow+1,kGray+2,kMagenta,kBlue+2};//{9,kCyan+2,45,kMagenta,kGray+1,kRed,kBlue+2,kMagenta,kCyan};
int line_color1[9]= {kRed,kRed,kRed+2,kViolet+2,kRed-2,kYellow+1,kGray+2,kMagenta,kBlue+2};
int line_color2[9] = {kRed,kRed,kBlue,kRed+2,kGray,kViolet+2,kRed-2,kYellow+1,kGray+2};//,kMagenta};
//int line_color[9] = {kMagenta+2, kGray+2, kRed, kRed+2, kMagenta, kRed - 3, kBlue + 2 , kCyan + 1 , kRed + 3 };
vector<int> col={kRed,kRed,kViolet,kGray,kViolet+2,kRed-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
...
void decorate(TH1F* hist,int i, int j){
  ...
  //   hist->SetFillColor(col[i]);
   ...
   vector<int> Style;
   if(j==1){
     col={kRed,kViolet,kRed+2,kViolet+2,kRed-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
     Style= {1001,3008,3019,3244};

   }
   else if(j==2){
     col={kRed,kBlue,kRed+2,kBlue,kRed+2,kGray+1,kViolet+2,kRed-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
     Style={1001,1001,3008,1001,3244};
   }
   else if(j==3){
     col={kRed,kViolet,kBlue,kRed+2,kRed+2,kBlue,kViolet,kGray,kViolet+2,kRed-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
...
void finaloneD_Pred_LL(string pathname, int which_Lept, int which_TFBins, bool all, string string_sys)
{
  ...
  // }
  // if(which_Lept==3){
  sprintf(string_png,"Zinv_%s_%s",TFbins_str, string_sys.c_str());
  sprintf(hname,"%s",string_png);
  baseline1 = {"h_Sbins_LL_newSbins_v7_Pred_SR","Mu_CR"};//,"Elec_SR","Mu_SR","TauHad_SR"};//,"Mu_SR","Elec_SR"};
  ...
  if(which_Lept==3){
      baseline1 = {"h_Sbins_LL_newSbins_v7_Pred_SR","newSbins_Validation_v7_Mu_CR","newSbins_v7_TauHad_SR","newSbins_v7_Mu_SR","newSbins_v7_Elec_SR"};
  ...
  //   {
  f[0] = new TFile("FullRun2_totalZtoNuNu_PhoIdloose_phopt40_MET200.root");//out_Data_FullRun2_Allruns_MET_phoID_loose_pt40_MET200.root");
  ...
  n_files=2;
   ...
   //       f[1] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/WithBtagging/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloose_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //    }
  f[1] = new TFile("Sys_Pred_Zinv_Default_puSysUp.root");
  f[2] = new TFile("Sys_Pred_Zinv_Default_JetSys_JECup.root");
  f[3] = new TFile("Sys_Pred_Zinv_Default_JetSys_JERup.root");
  f[4] = new TFile("Sys_Pred_Zinv_Default_btagSFup.root");

  // f[1] = new TFile("Sys_Pred_Zinv_Default_btag_SF_up.root");
  // f[2] = new TFile("Sys_Pred_Zinv_Default_JEC_Sys_up.root");
  // f[3] = new TFile("Sys_Pred_Zinv_Default_JER_Sys_up.root");
  // f[4] = new TFile("Sys_Pred_Zinv_Default_CrossSection_Sys_up.root");
  // f[5] = new TFile("Sys_Pred_Zinv_Default_Pileup_Sys_up.root");
  ...
  for(int i_file=4; i_file<5;i_file++)
	...
	sysError = sqrt(TF_new[1]*TF_new[1]+TF_new[2]*TF_new[2]+TF_new[3]*TF_new[3]+TF_new[4]*TF_new[4]+TF_new[5]*TF_new[5]);//+0.03*0.03+0.12*0.12);
      	...
      	// TF_new[0] = h_TF_new[1]->GetBinContent(j);
      	 ...
      	 // ratio = delta/TF_orig;
      	cout<<j<<"\t"<<ratio<<"\t"<<TF_orig<<"\t"<<TF_new[0]<<"\t"<<sysError<<"\t"<<endl;
