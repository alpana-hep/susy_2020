...
//    c->SetBottomMargin(0.12);
  ...
  for(int i =0;i<(int)hist.size(); i ++) {
    ...
    if(normalize) {
      ...
      hist.at(i)->GetYaxis()->SetTitle("Purity factor (#beta)");
    }
  ...
  //  if(ymax<=10) ymax=10;
  for(int i = 0;i<(int)hist.size(); i++) {
    ymax = hist.at(i)->GetMaximum();
    ymin = hist.at(i)->GetMinimum();
    
    if(!normalize) {
      // if(ymax>0.05)
      // 	hist.at(i)->GetYaxis()->SetRangeUser(ymin,0.1);
      // else
	 hist.at(i)->GetYaxis()->SetRangeUser(ymin,2.0*ymax);}//10000.0*ymax);
    else
...
//     Tl.SetTextSize(0.055);
  ...
  gPad->Modified();
    ...
    hist_ratio->GetYaxis()->SetTitle("#frac{#beta^{Data}}{#beta^{MC}}");//TF = #frac{N_{SR}}{N_{CR}}");//(0#mu,1#gamma)}{(1#mu,1#gamma)}");
   ...
   hist_ratio->Draw("");
   l->Draw("sames");
   TLine *l1 =new TLine(xmin,0.8,xrange,0.8);
   l1->SetLineStyle(7);
   l1->Draw("sames");
   TLine *l2 =new TLine(xmin,1.2,xrange,1.2);
   l2->SetLineStyle(7);
...
void purityFactor_Plots(string pathname, int which_Lept)
{
  ...
  //if(which_Lept==1){
    ...
    legend_texts ={"Data","MC"};//f(Data:single e)","f(MC: Z(ee))"};//,"#tau-had SR","lost #mu SR","lost e SR","(1l,1#gamma) CR","Failed Iso"};
    
    //sprintf(hname,"");
    if(which_Lept==0){
    sprintf(string_png,"purityFactor_Data_MC_%s",TFbins_str);
    sprintf(hname,"%s_phoID_loose_29Jan24",string_png);
    }
    else if (which_Lept==1){
      sprintf(string_png,"purityFactor_Data_CRvsSR_%s",TFbins_str);
      sprintf(hname,"%s_phoID_loose_29Jan24",string_png);
    }
  ...
  //   {
    ...
    //   }
    f[0]= new TFile("./out_purityFactor_Calc_MC_Default.root");
    f[1]= new TFile("./out_purityFactor_Calc_Data_Default.root");
  ...
  vector<double> xmax={1500,1500,11,8,200,600,20,10,10,10,100,1000,1,50,0.5,5,5,1000,1,5,5,1000,1,5,5,1000,1,5,5,1000,1,5,5,1000,2,120,400,3,600,5,5,600};
  ...
  vector<string> filetag=  {"2018","2017","2016postVFP","FullRun2","2016preVFP","2016"};
  vector<string> filetag1= {"2018","2017","2016postVFP","FullRun2","2016preVFP","2016"};//  {"2018","2017","2016postVFP","FullRun2","MC_2016preVFP","MC_2016"};
  ...
  energyy={59.74,41.529,16.5,137.19,19.59,36};   
 ...
 //  }
  ...
  for(int i_file=0; i_file<6;i_file++)
      ...
      for(int i_cut=varName.size()-5; i_cut<varName.size()-4;i_cut++)
	{
	  ...
	  cout<<h_resp->Integral()<<"\t"<<f[1]->GetName()<<endl;	 
	  cout<<" hist_list_Njets.size() "<<hist_list_Njets.size()<<"\t "<<endl;//hist_list_Bjets.size() "<<hist_list_Bjets.size()<<endl;
	  cout<<"MC: purity factor "<<endl;
	  for(int i=0 ; i<h_resp->GetNbinsX() ; ++i) {
            if(h_resp->GetBinContent(i)!=0)
	    cout<<h_resp->GetBinContent(i)<<"\t"<<h_resp->GetBinError(i)<<endl;
          }
	  cout<<"Data: purity factor "<<endl;
          for(int i=0 ; i<h_resp2->GetNbinsX() ; ++i) {
            if(h_resp2->GetBinContent(i)!=0)
            cout<<h_resp2->GetBinContent(i)<<"\t"<<h_resp2->GetBinError(i)<<endl;
          }
	  ...
	  sprintf(full_path,"%s/PurityFactorPlots_%s_%s_%s",pathname.c_str(),string_png,varName[i_cut].c_str(),filetag2[i_file].c_str());
