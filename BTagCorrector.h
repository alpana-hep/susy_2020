...
class BTagCorrector {
  ...
  BTagCorrector() : debug(false), fastsim(false), 
		    btagSFunc("up"), mistagSFunc("up"), btagCFunc("up"), ctagCFunc("up"), mistagCFunc("up"), 
		    //    btagSFunc("up"), mistagSFunc("central"), btagCFunc("central"), ctagCFunc("central"), mistagCFunc("central"),
		    //btagSFunc("central"), mistagSFunc("central"), btagCFunc("central"), ctagCFunc("central"), mistagCFunc("central"),
		    // mistagSFunc("down"), btagCFunc("down"), ctagCFunc("down"), mistagCFunc("down"), 
		...
		void SetEffs(TFile* file){
		  ...
		  if(!h_eff_b){
		    ...
		    TH2F* d_eff_b = (TH2F*)file->Get("d_eff_b_");
		    h_eff_b = (TH2F*)n_eff_b->Clone("h_eff_b");
		    // cout<<"!h_eff_b : "<<n_eff_b->GetBinContent(0,0)<<endl;
		...
		void SetCalib(string cfile){
		  ...
		  //cout<<"initialize btag helper classes"<<"\t"<<cfile<<endl;
		  calib = BTagCalibrationS("",cfile);
		  //cout<<"initialize btag helper classes"<<endl;
		  reader = BTagCalibrationReaderS(BTagEntryS::OP_MEDIUM, "central", {"up","down"});
		  //cout<<"initialize btag helper classes"<<endl;		  
		  reader.load(calib, BTagEntryS::FLAV_B, "comb");
		  //cout<<"initialize btag helper classes"<< "\t"<<BTagEntryS::FLAV_B<<endl;
		  reader.load(calib, BTagEntryS::FLAV_C, "comb");
		  //cout<<"initialize btag helper classes"<< "\t"<<BTagEntryS::FLAV_C<<endl;
		  reader.load(calib, BTagEntryS::FLAV_UDSG, "incl");
		  //cout<<"initialize btag helper classes"<<endl;
		}
		...
		//method 1b
  vector<double> GetCorrections(vector<TLorentzVector> Jets, vector<int> Jets_flavor, vector<bool> Jets_HTMask, int uu){
    ...
    SetBtagSFunc(uu);
		  ...
		  for(unsigned ja = 0; ja < Jets.size(); ++ja){
		    ...
		    InitSFEff(Jets[ja].Pt(), Jets[ja].Eta(), Jets_flavor[ja], sfEffLists[ja],uu);
		    //cout<<sfEffLists[ja][0]<<"\t"<<sfEffLists[ja][1]<<"\t"<<sfEffLists[ja][2]<<endl;
		    double eps_a = sfEffLists[ja][0]*sfEffLists[ja][1]*sfEffLists[ja][2];
				
		    //jet index, pt, eta, flavor, eff, sf, cf
		    if(debug) cout << "Jet " << ja << ": " << Jets[ja].Pt() << ", " << fabs(Jets[ja].Eta()) << ", " << abs(Jets_flavor[ja])<< ", " << sfEffLists[ja][0] << ", " << sfEffLists[ja][1] << ", " << sfEffLists[ja][2] << endl;
		    ...
		    for(unsigned jb = 0; jb < Jets.size(); ++jb){
		      ...
		      //get sf and eff values (checks if already calculated)
		      InitSFEff(Jets[jb].Pt(), Jets[jb].Eta(), Jets_flavor[jb], sfEffLists[jb],uu);
		      double eps_b = sfEffLists[jb][0]*sfEffLists[jb][1]*sfEffLists[jb][2];
				  
		      //jet index, pt, eta, flavor, eff, sf, cf
		      if(debug) cout << "\tJet " << jb << ": " << Jets[jb].Pt() << ", " << fabs(Jets[jb].Eta()) << ", " << abs(Jets_flavor[jb])<< ", " << sfEffLists[jb][0] << ", " << sfEffLists[jb][1] << ", " << sfEffLists[jb][2] << endl;
			...
			//get sf and eff values (checks if already calculated)
			InitSFEff(Jets[jc].Pt(), Jets[jc].Eta(), Jets_flavor[jc], sfEffLists[jc],uu);
			double eps_c = sfEffLists[jc][0]*sfEffLists[jc][1]*sfEffLists[jc][2];
						
			//jet index, pt, eta, flavor, eff, sf, cf
			if(debug) cout << "\t\tJet " << jc << ": " << Jets[jc].Pt() << ", " << fabs(Jets[jc].Eta()) << ", " << abs(Jets_flavor[jc])<< ", " << sfEffLists[jc][0] << ", " << sfEffLists[jc][1] << ", " << sfEffLists[jc][2] << endl;
		...
		//method 1a
  double GetSimpleCorrection(vector<TLorentzVector> Jets, vector<int> Jets_flavor, vector<bool> Jets_HTMask, vector<double> Jets_bDiscriminator, double wp,int uu){
			//result
			...
			for(unsigned ja = 0; ja < Jets.size(); ++ja){
				...
				if(debug)
				  cout << "Jet " << ja << ": " << Jets[ja].Pt() << ", " << fabs(Jets[ja].Eta()) << ", " << abs(Jets_flavor[ja])  << ", " << Jets_bDiscriminator[ja]
								<< ", " << sfEffLists[ja][0] << ", " << sfEffLists[ja][1] << ", " << sfEffLists[ja][2] << endl;
							
				if(Jets_bDiscriminator[ja] > wp){
					c_numer *= eff_a;
					c_denom *= eff_a*sf_a;
				}
				else {
			...
			//			cout<<"final "<<c_numer<<"\t"<<c_denom<<"\t"<<wp<<"\t"<<c_numer/c_denom<<endl;
			return c_numer/c_denom;
		}
		
		//helper function
  void InitSFEff(double pt, double eta, int flav, vector<double>& sfEffList, int uu){
			//avoid rerunning this
			if(sfEffList.size()>0) return;
			//			cout<<uu<<"\t"<<btagSFunc<<"\t"<<endl;
			string btagSFuncc="";
			string mistagSFuncc("up"), btagCFuncc("up"), ctagCFuncc("up"), mistagCFuncc("up");
			if (uu == 0) {
			  btagSFuncc = "central";
			  mistagSFuncc="central";
			  btagCFuncc = "central";
			  ctagCFuncc= "central";
			  mistagCFuncc ="central";
			} else if (uu == 1) {
			  btagSFuncc = "up";
			     mistagSFuncc="up";
                          btagCFuncc = "up";
			...
			//			cout<<uu<<"\t"<<btagSFuncc<<"\t"<<endl;
			//btagSFunc=btagSFuncc;
			
			//			mistagSFunc("up"), btagCFunc("up"), ctagCFunc("up"), mistagCFunc("up");
			//btagSFunc = u==0 ? "central" : (u==1 ? "up" : "down");
			//SetBtagSFunc(uu);
			//use abs(eta) for now
			eta = fabs(eta);
			//use abs(flav) always
			...
			if(flav==5){ //b-tag
				...
				sfEffList[1] = reader.eval_auto_bounds(btagSFuncc,BTagEntryS::FLAV_B,eta,pt);
				//cout<<"inside initSF "<<"\t"<<sfEffList[0]<<"\t"<<sfEffList[1]<<"\t"<<flav<<endl;
				if(fastsim){
					sfEffList[2] = readerFast.eval_auto_bounds(btagCFuncc,BTagEntryS::FLAV_B,eta,pt);
				}
			}
			else if(flav==4){ //charm mistag
				sfEffList[0] = h_eff_c->GetBinContent(h_eff_c->FindBin(pt,eta));
				sfEffList[1] = reader.eval_auto_bounds(btagSFuncc,BTagEntryS::FLAV_C,eta,pt);
				if(fastsim){
					sfEffList[2] = readerFast.eval_auto_bounds(ctagCFuncc,BTagEntryS::FLAV_C,eta,pt);
				}
			}
			else if(flav<4 || flav==21){ //udsg mistag
				sfEffList[0] = h_eff_udsg->GetBinContent(h_eff_udsg->FindBin(pt,eta));
				sfEffList[1] = reader.eval_auto_bounds(mistagSFuncc,BTagEntryS::FLAV_UDSG,eta,pt);
				if(fastsim){
					sfEffList[2] = readerFast.eval_auto_bounds(mistagCFuncc,BTagEntryS::FLAV_UDSG,eta,pt);
				}
			}
		}
