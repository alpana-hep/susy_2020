...
class AnalyzeLightBSM : public NtupleVariables{
 ...
 public:
  ...
  /* int BTags; */
  int GenElectrons_,GenParticles_, GenMuons_,GenTaus_,GenJets_,Electrons_,Photons_,Muons_,Taus_, Jets_;
  bool isSignal=false;
...
void AnalyzeLightBSM::BookHistogram(const char *outFileName, const char *N2_mass) {
  ...
  char check1[1000];
  for(int c =0; c<2;c++)//checks.size();c++)
    {
...
char hist_name1[1000];
 ...
 for(int i=0;i<2;i++)//baseline.size();i++)
    {
