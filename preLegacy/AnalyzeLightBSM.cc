...
void AnalyzeLightBSM::EventLoop(const char *data,const char *inputFileList, const char *sample , const char *outFileName, const char *elec, const char* phoID) {
 ...
 //   }
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)
	...
	GenElectrons_ = GenElectrons->size();
	GenParticles_ = GenParticles->size();
	GenMuons_ = GenMuons->size();
	GenTaus_  = GenTaus->size();
	GenJets_ = GenJets->size();
	Electrons_ = Electrons->size();
	...
	for(int i=0; i<Electrons->size();i++)
	{
	  ...
	  Electrons_v1.push_back((*Electrons)[i]);
      ...
      for(int i=0; i<GenElectrons->size();i++)
	{
	  ...
	  GenElectrons_v1.push_back((*GenElectrons)[i]);
	}
      if(Debug)
      ...
      for(int i=0; i<GenJets->size();i++)
	{
	  ...
	  GenJets_v1.push_back((*GenJets)[i]);
	  ...
	  GenMuons_v1.push_back((*GenMuons)[i]);
	...
	GenTaus_v1.push_back((*GenTaus)[i]);
      }
	  ...
	  GenParticles_v1.push_back((*GenParticles)[i]);
	  // if(Debug)
      ...
      for(int i=0;i<Photons->size();i++)
	{
	  ...
	  Photons_v1.push_back((*Photons)[i]);
	}
      if(Debug)
	...
	cout<<"cp - before gen particles"<<endl;
	  ...
	  //     cout<<GenParticles_v1[igen].Pt()<<"\t"<<GenParticles_v1[igen].Eta()<<endl;}
	...
	cout<<nelec_reco<<endl;
	  ...
	  leadGenPhoIdx_v1 = iPho;
    	  goodPhoIndx_n.push_back(iPho);
    	}
      }
      sortTLorVec(&goodPho_n);
      int nPhotons = goodPho_n.size();
      TLorentzVector bestPhoton=getBestPhoton(pho_ID);      
      if(Debug && Photons_!=0 && leadGenPhoIdx_v1!=-100)
	cout<<" bestPhoton.Pt()  "<<"\t"<<bestPhoton.Pt() <<"\t"<<"goodPho_n[0].Pt()  "<<goodPho_n[0].Pt()<<endl;
    ...
    h_selectBaselineYields_->Fill("hadJets.size() ",wt);
    if(Debug)
