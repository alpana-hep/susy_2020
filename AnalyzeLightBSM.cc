...
void AnalyzeLightBSM::EventLoop(const char *data,const char *inputFileList, const char *sample , const char *outFileName, const char *elec, const char* phoID) {
 ...
 //  // counters for events yields after each selection/rejection
  ...
  sprintf(hname, "TF_Muon_LLEstimation_binsV0_phoID_loose_08Aug23.root");//,phoID);
  char* hname1 = new char [200];
  sprintf(hname1, "TF_Electron_LLEstimation_binsV0_phoID_loose_08Aug23.root");//,phoID);
  char* hname2 = new char [200];
  sprintf(hname2, "TF_allin1_LLEstimation_binsV0_phoID_loose_08Aug23.root");//,phoID);
 ...
 //   }
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)
    ...
    //   //selecting electron and muons
      ...
      // }
      bool flag_looseID=false, flag_mvaID=false;
      for(int iPho=0;iPho<Photons_;iPho++){
    	if(abs(Photons_v1[iPho].Eta())<2.4 && ((*Photons_hasPixelSeed)[iPho]<0.001)) {
	  if((*Photons_fullID)[iPho] )
	    flag_looseID= true;
	  if((*Photons_mvaValuesID)[iPho]>-0.02)
	    flag_mvaID = false;
	  if(flag_looseID && flag_mvaID)
	    h_mvaVsloose->SetBinContent(1,1,wt);
	  else if (!flag_mvaID && flag_looseID)
	    h_mvaVsloose->SetBinContent(1,0,wt); 
	  else if (flag_mvaID && !flag_looseID)  
	    h_mvaVsloose->SetBinContent(0,1,wt); 
	  else if (!flag_mvaID && !flag_looseID)
	    h_mvaVsloose->SetBinContent(0,0,wt);
    ...
    // if(bestPhoton.Pt()>40 )
    //   cout<<"Error in best photon index "<<bestPhotonIndxAmongPhotons<<"\t"<<(*Photons_hasPixelSeed)[bestPhotonIndxAmongPhotons]<<"\t"<<(*Photons_nonPrompt).at(bestPhotonIndxAmongPhotons)<<"\t"<<(*Photons_electronFakes).at(bestPhotonIndxAmongPhotons)<<endl;
	...
	cout<<"in Electron CR + with 1photon ====== "<<jentry<<endl;
      ...
      if(MET>100){
      FillHistogram_Kinematics(6, nHadJets, BTags, bestPhoton.Pt(),mTPhoMET,dPhi_PhoMET,ST,wt_LL);
    ...
    if (!lost_elec_flag && nelec_reco == 0 && nmu_reco == 1){
      ...
      mu_CR = true;
      if(MET>100){     
      FillHistogram_Kinematics_varBin(4,nHadJets, BTags, bestPhoton.Pt(),ST,wt_LL);
	...
	h_mindR_Vs_pTratio_elec_pho[4]->Fill( bestPhoton.DeltaR(Muons_v1[mu_index]),Muons_v1[mu_index].Pt()/bestPhoton.Pt(),wt);}
      if(!s_sample.Contains("data")){
	if(flag_phoOrigin==0)
        FillHistogram_Kinematics(31,nHadJets,BTags,bestPhoton.Pt(),mTPhoMET,dPhi_PhoMET,ST,wt);
      else if(flag_phoOrigin==1)
    ...
    if (!s_sample.Contains("data") && lost_elec_flag && nelec_reco == 0 && nmu_reco == 0){
      ...
      //if(fakeElectron) continue;// shoudl contribute in FR estimation
	...
	if(Elec_passIso) genElec_passIso=true; else genElec_passIso=false;
	}
      }    
    }
...
//mu_SR
	...
	fakeElectron =(proc && (bestPhoton.DeltaR(v_genEle2[0])<0.2 && v_genEle2[0].Pt()/bestPhoton.Pt()<1.2 && v_genEle2[0].Pt()/bestPhoton.Pt()>0.8))||(proc1 && ((bestPhoton.DeltaR(v_genEle2[0])<0.2 && v_genEle2[0].Pt()/bestPhoton.Pt()<1.2 && v_genEle2[0].Pt()/bestPhoton.Pt()>0.8)|| (v_genEle2.size()>1 && (bestPhoton.DeltaR(v_genEle2[1])<0.2 && v_genEle2[1].Pt()/bestPhoton.Pt()<1.2 && v_genEle2[0].Pt()/bestPhoton.Pt()>0.8))));}
      ...
      if(((s_sample.Contains("WGJets") || s_sample.Contains("WJets"))&& nGentau_had1>0) ||( (s_sample.Contains("TTGJets") || s_sample.Contains("TTJets")) && nGentau_had1>0))
	tauHad_SR = true;
	...
	if(genMu_passAccep){ if(Muons_v1.size()!=0) dR = MinDr(v_genMu2[i],Muons_v1);
	  ...
	  if(Mu_passIso) genMu_passIso=true; else genMu_passIso=false;
        }
      }
    }
	...
	if(!genElec_passpTcut)  FillHistogram_Kinematics(13,nHadJets,BTags,bestPhoton.Pt(),mTPhoMET,dPhi_PhoMET,ST,wt);
      ...
      else if(!genElec_passId) FillHistogram_Kinematics(10,nHadJets,BTags,bestPhoton.Pt(),mTPhoMET,dPhi_PhoMET,ST,wt);
      if(!genElec_passIso) FillHistogram_Kinematics(11,nHadJets,BTags,bestPhoton.Pt(),mTPhoMET,dPhi_PhoMET,ST,wt); }
	...
	if(!genMu_passpTcut)  FillHistogram_Kinematics(19,nHadJets,BTags,bestPhoton.Pt(),mTPhoMET,dPhi_PhoMET,ST,wt);
      ...
      else if(!genMu_passId) FillHistogram_Kinematics(16,nHadJets,BTags,bestPhoton.Pt(),mTPhoMET,dPhi_PhoMET,ST,wt);
      if(!genMu_passIso) FillHistogram_Kinematics(17,nHadJets,BTags,bestPhoton.Pt(),mTPhoMET,dPhi_PhoMET,ST,wt); }
	 }
...
TLorentzVector AnalyzeLightBSM::getBestPhoton(int pho_ID){
  ...
  for(int iPho=0;iPho<Photons_;iPho++){
    ...
    //////////////////////////// REMINDER   ///////////////////////////////////
    //REMOVE (*Photons_fullID)[iPho] BEFORE SUBMITTING THE JOBS FOR NEXT TEST
    bool isBarrelPhoton=false;
    bool isEndcapPhoton=false;    
    double PhEta=Photons_v1[iPho].Eta();//.eta();
    ...
    if(abs(Photons_v1[iPho].Eta())<2.5 && (*Photons_hasPixelSeed)[iPho]<0.001 && ( ((*Photons_fullID)[iPho] && pho_ID==0) || (pho_ID==3 && (isBarrelPhoton && (*Photons_mvaValuesID)[iPho]>-0.02) || (isEndcapPhoton && (*Photons_mvaValuesID)[iPho]>-0.26)) || (pho_ID==4 && (isBarrelPhoton && (*Photons_mvaValuesID)[iPho]>0.42) || (isEndcapPhoton && (*Photons_mvaValuesID)[iPho]>0.14))))
      // (pho_ID==1 &&(((*Photons_cutBasedID)[iPho]==1 || (*Photons_cutBasedID)[iPho]==2))) || (pho_ID==2 && (*Photons_cutBasedID)[iPho]==2) || (pho_ID==3 && (*Photons_mvaValuesID)[iPho]>-0.02) || (pho_ID==4 && (*Photons_mvaValuesID)[iPho]>0.42)))
      {
	// if(Photons_v1[iPho].Pt()>40)
	//   cout<<"cutebased ID "<<(*Photons_cutBasedID)[iPho]<<"\t"<<(*Photons_hasPixelSeed)[iPho]<<"\t"<<(*Photons_fullID)[iPho]<<"\t"<<pho_ID<<endl;
...
void AnalyzeLightBSM::FillHistogram_Kinematics(int i, int Njets, int btags, double pho_Pt, double mt_phoMET, double dPhi, double ST, double wt){
  ...
  //  h_TFbins_LL[i]->Fill(TFbins_v2,wt);
  h_TFbins_LL_v2[i]->Fill(TFbins_v1,wt);
  h_Sbins_LL[i]->Fill(searchBin,wt);
  h_TFbins_LL_v1[i]->Fill(TFbins_v2,wt);
...
int AnalyzeLightBSM::Photons_OriginType(){
  //  cout<<"inside Photons_OriginType "<<bestPhotonIndxAmongPhotons<<"\t"<<(*Photons_nonPrompt).at(bestPhotonIndxAmongPhotons)<<"\t"<<(*Photons_electronFakes).at(bestPhotonIndxAmongPhotons)<<"\t"<<hasGenPromptPhoton<<endl;
  int flag_photon = -1;
  if(!(*Photons_nonPrompt).at(bestPhotonIndxAmongPhotons) && !(*Photons_electronFakes).at(bestPhotonIndxAmongPhotons) && hasGenPromptPhoton)
    ...
    flag_photon = 3;
  //  cout<<"inside Photons_OriginType "<<bestPhotonIndxAmongPhotons<<"\t"<<(*Photons_nonPrompt).at(bestPhotonIndxAmongPhotons)<<"\t"<<(*Photons_electronFakes).at(bestPhotonIndxAmongPhotons)<<"\t"<<hasGenPromptPhoton<<"\t"<<flag_photon<<endl;
  return flag_photon;  
}
