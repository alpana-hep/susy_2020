...
// counters for events yields after each selection/rejection
 ...
 if(lost_elec_flag)
   {
     sprintf(histname,"h_LL_TFbins_MEtNhadJetsBjetsBins_Elec__%s_%s",sample1,data);
   ...
   {
     sprintf(histname,"h_LL_TFbins_MEtNhadJetsBjetsBins_FailAcep_TauHadronic_%s_%s",sample1,data);
 ...
 // nentries=100000;
  for (Long64_t jentry=0; jentry<nentries;jentry++)
	...
	wt = (0.00887*lumiInfb*1000)/nentries;
      ...
      else if(s_sample.Contains("WGJets")) //(s_data.Contains("2016") || s_data.Contains("2017") || s_data.Contains("2018") || s_sample.Contains("WGJets"))
	{
    ...
    //    cout<<"Event:  "<<jentry<<"\t"<<(*Photons_cutBasedIDFall17V2).at(bestPhotonIndxAmongPhotons)<<"\t"<<(*Photons_mvaValuesIDFall17V2).at	\
      (bestPhotonIndxAmongPhotons)<<"\t"<<endl;
   ...
   // int nCR_elec =0,nCR_mu=0,nCR_Tau=0,nSR_elec =0,nSR_mu=0,nSR_Tau=0, FailIso_Elec=0,FailIso_Mu=0, FailAccept_Elec=0,FailAccept_Mu=0, FailId_Elec=0,FailId_Mu=0, PassIso_Elec=0,PassIso_Mu=0, PassAccept_Elec=0,PassAccept_Mu=0, PassId_Elec=0,PassId_Mu=0, nfakeRatePho=0;
	...
	//cout<<fakeElectron<<"\t"<<proc<<"\t"<<proc1<<"\t"<<(bestPhoton.DeltaR(v_genEle2[0]))<<"\t"<<v_genEle2[0].Pt()/bestPhoton.Pt()<<endl;			
	if(!(*Photons_nonPrompt).at(bestPhotonIndxAmongPhotons) && !fakeElectron && hasGenPromptPhoton)//(*Photons_genMatched).at(bestPhotonIndxAmongPhotons))          
	    ...
	    // // if(bestPhoton.DeltaR(v_genEle2[0])<0.2 && v_genEle2[0].Pt()/bestPhoton.Pt()<1.2 && v_genEle2[0].Pt()/bestPhoton.Pt()>0.8)
	    // //   {
	    // 	cout<<"event:   "<<"\t"<<jentry<<"\t"<<bestPhotonIndxAmongPhotons<<"\t"<<bestPhoton.DeltaR(v_genEle2[0])<<"\t"<<v_genEle2[0].Pt()/bestPhoton.Pt()<<"\t"<<v_genEle2[0].Pt()<<"\t"<<v_genEle2.size()<<"\t"<<endl;
	    // 	cout<<"Gen_ elec - FR ====================="<<endl;
	    // 	for(int i=0;i<GenParticles->size();i++)
	    // 	  {
	    // 	    if((*GenParticles)[i].Pt()!=0){
	    // 	      cout<<abs((*GenParticles_PdgId)[i])<<"\t"<<abs((*GenParticles_ParentId)[i])<<"\t"<<(*GenParticles_Status)[i]<<"\t"<<(*GenParticles)[i].Pt()<<endl;
	    // 	      if(abs((*GenParticles_PdgId)[i])==11)
	    // 		{
	    // 		  cout<<"========= checks with gen particles ========"<<endl;

	    // 		  cout<<bestPhoton.DeltaR((*GenParticles)[i])<<"\t"<<(*GenParticles)[i].Pt()/bestPhoton.Pt()<<"\t"<<endl;
	    // 		}
	    // 	    }
	    // 	  }
	    // 	//}
	...
	// if(bestPhoton.DeltaR(v_genEle2[0])>0.2 || !(v_genEle2[0].Pt()/bestPhoton.Pt()<1.1 && v_genEle2[0].Pt()/bestPhoton.Pt()>0.9))
	//   {
        //     cout<<"event:   "<<"\t"<<jentry<<"\t"<<bestPhotonIndxAmongPhotons<<"\t"<<bestPhoton.DeltaR(v_genEle2[0])<<"\t"<<v_genEle2[0].Pt()/bestPhoton.Pt()<<"\t"<<v_genEle2[0].Pt()<<"\t"<<v_genEle2.size()<<"\t"<<endl;
        //     cout<<"Gen_ elec - FR ====================="<<endl;
        //     for(int i=0;i<GenParticles->size();i++)
	...
	// 	      cout<<bestPhoton.DeltaR((*GenParticles)[i])<<"\t"<<(*GenParticles)[i].Pt()/bestPhoton.Pt()<<"\t"<<endl;
        ...
        //       }
          ...
          //   for(int i=0;i<Photons_genMatched->size();i++)
          //     {
          //       cout<<jentry<<"\t"<<"promptPhoton:   "<<"\t"<<(*Photons)[i].Pt()<<"\t"<<(*Photons_nonPrompt).at(i)<<"\t"<<(*Photons_genMatched).at(i)<<"\t"<<(*Photons_electronFakes).at(i)<<"\t"<<hasGenPromptPhoton<<"\t"<<"FullID"<<"\t"<<(*Photons_fullID)[i]<<endl;
          //     }
	  // }

	// if((*Photons).size()>1){
	// cout<<"event:   "<<"\t"<<jentry<<"\t"<<bestPhotonIndxAmongPhotons<<"\t"<<bestPhoton.DeltaR(v_genEle2[0])<<"\t"<<v_genEle2[0].Pt()/bestPhoton.Pt()<<"\t"<<v_genEle2.size()<<"\t"<<endl;
	// cout<<"elec -sr FR ====================="<<endl;
	// for(int i=0;i<Photons_genMatched->size();i++)
	//   {
	//     cout<<jentry<<"\t"<<"promptPhoton:   "<<"\t"<<(*Photons)[i].Pt()<<"\t"<<(*Photons_nonPrompt).at(i)<<"\t"<<(*Photons_genMatched).at(i)<<"\t"<<(*Photons_electronFakes).at(i)<<"\t"<<hasGenPromptPhoton<<"\t"<<"FullID"<<"\t"<<(*Photons_fullID)[i]<<"\t"<<(*Photons_hasPixelSeed)[i]<<endl;
	//   }
	// }
	if(recojetmindr<0.3)
	  {
	...
	bool fakeElectron=false;
	cout<<"before fake elctron"<<endl;
	if(v_genEle2.size()!=0)
	  {
	    fakeElectron =(proc && v_genEle2.size()!=0 && (bestPhoton.DeltaR(v_genEle2[0])<0.2 && v_genEle2[0].Pt()/bestPhoton.Pt()<1.2 && v_genEle2[0].Pt()/bestPhoton.Pt()>0.8));//||(proc1 && v_genEle2.size()!=0 &&((bestPhoton.DeltaR(v_genEle2[0])<0.2 && v_genEle2[0].Pt()/bestPhoton.Pt()<1.2 && v_genEle2[0].Pt()/bestPhoton.Pt()>0.8)|| (v_genEle2.size()>1 && (bestPhoton.DeltaR(v_genEle2[1])<0.2 && v_genEle2[1].Pt()/bestPhoton.Pt()<1.2 && v_genEle2[1].Pt()/bestPhoton.Pt()>0.8))));
	
cout<<jentry<<"\t"<<fakeElectron<<"\t"<<proc<<"\t"<<proc1<<"\t"<<(bestPhoton.DeltaR(v_genEle2[0]))<<"\t"<<v_genEle2[0].Pt()/bestPhoton.Pt()<<endl;
	  }
	if(Debug)
	...
	if(((s_sample.Contains("WGJets") || s_sample.Contains("WJets"))&& nGentau_had1>0) ||( (s_sample.Contains("TTGJets") || s_sample.Contains("TTJets")) && nGentau_had1>1))
	...
	else
	  {
	    ...
	    if(!(*Photons_nonPrompt).at(bestPhotonIndxAmongPhotons) && !fakeElectron && hasGenPromptPhoton)//!(*Photons_electronFakes).at(bestPhotonIndxAmongPhotons) && hasGenPromptPhoton)
	      {
...
TLorentzVector AnalyzeLightBSM::getBestPhoton(int pho_ID){
  ...
  for(int iPho=0;iPho<Photons->size();iPho++){
    if( (*Photons_fullID)[iPho] && ((*Photons_hasPixelSeed)[iPho]<0.001) &&( pho_ID==0 || (pho_ID==1 && (((*Photons_cutBasedIDFall17V2)[iPho]==1 || (*Photons_cutBasedIDFall17V2)[iPho]==2))) || (pho_ID==2 && (*Photons_cutBasedIDFall17V2)[iPho]==2) || (pho_ID==3 && (*Photons_mvaValuesIDFall17V2)[iPho]>-0.02) || (pho_ID==4 && (*Photons_mvaValuesIDFall17V2)[iPho]>0.42)) ) {
