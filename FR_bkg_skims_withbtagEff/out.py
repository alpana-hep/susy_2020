...
---
> // Tue Aug 15 10:36:56 2023 by ROOT version 6.18/04
5c5
...
---
> // found on file: root://cmseos.fnal.gov//store/user/lpcsusyhad/SusyRA2Analysis2015/Run2ProductionV20/Run2016F-UL2016-v2/MET/0_RA2AnalysisTree.root
7d6
...
>    static constexpr Int_t kMaxJets = 67;
>    static constexpr Int_t kMaxJetsAK15 = 16;
>    static constexpr Int_t kMaxJetsAK15_subjets = 18;
>    static constexpr Int_t kMaxJetsAK8 = 46;
>    static constexpr Int_t kMaxJetsAK8_subjets = 84;
>    static constexpr Int_t kMaxJetsConstituents = 1290;
>    static constexpr Int_t kMaxMuons = 4;
>    static constexpr Int_t kMaxPhotons = 2;
>    static constexpr Int_t kMaxTAPElectronTracks = 5;
>    static constexpr Int_t kMaxTAPMuonTracks = 5;
>    static constexpr Int_t kMaxTAPPionTracks = 41;
52,75c40,41
...
---
> #ifdef Alp_data_cxx
> Alp_data::Alp_data(TTree *tree) : fChain(0) 
> {
> // if parameter tree is not specified (or zero), connect the file
> // used to generate this class and read the Tree.
>    if (tree == 0) {
>       TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("root://cmseos.fnal.gov//store/user/lpcsusyhad/SusyRA2Analysis2015/Run2ProductionV20/Run2016F-UL2016-v2/MET/0_RA2AnalysisTree.root");
>       if (!f || !f->IsOpen()) {
>          f = new TFile("root://cmseos.fnal.gov//store/user/lpcsusyhad/SusyRA2Analysis2015/Run2ProductionV20/Run2016F-UL2016-v2/MET/0_RA2AnalysisTree.root");
>       }
>       TDirectory * dir = (TDirectory*)f->Get("root://cmseos.fnal.gov//store/user/lpcsusyhad/SusyRA2Analysis2015/Run2ProductionV20/Run2016F-UL2016-v2/MET/0_RA2AnalysisTree.root:/TreeMaker2");
>       dir->GetObject("PreSelection",tree);
1234,1238c736,738
...
---
> Int_t Alp_data::GetEntry(Long64_t entry)
> {
> // Read contents of entry.
>    if (!fChain) return 0;
>    return fChain->GetEntry(entry);
> }
> Long64_t Alp_data::LoadTree(Long64_t entry)
> {
> // Set the environment to read one entry
>    if (!fChain) return -5;
>    Long64_t centry = fChain->LoadTree(entry);
...
---
> void Alp_data::Show(Long64_t entry)
> {
> // Print contents of entry.
> // If entry is not specified, print current entry
>    if (!fChain) return;
>    fChain->Show(entry);
> }
> Int_t Alp_data::Cut(Long64_t entry)
> {
> // This function may be called from Loop.
> // returns  1 if entry is accepted.
> // returns -1 otherwise.
