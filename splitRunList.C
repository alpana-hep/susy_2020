...
void splitRunList(string infile,int nfPerJob, string datasetAna, string process, string LL, string phoID){
  ...
  string filesToTransfer = "map_crosssection_SMprocess.txt";//, TF_allin1_LLEstimation_electron_newTFbins.root, TF_allin1_LLEstimation_muon_newTFbins.root, TMVAClassification_BDT_100trees_2maxdepth.weights.xml";
  //---------------------------------------------------
  ...
  for(int i=0;i<jobid;i++){
    ...
    sprintf(logFile,"%s_phoID_%s_pt40_%s_job%i",dataset.c_str(),phoID.c_str(),LL.c_str(),i);
    outf.open(name);
	...
	<<"Transfer_Input_Files = "<<filesToTransfer<<","<<exeAna<<","<<fileListName<<","<<"TF_allin1_LLEstimation_electron_newTFbins.root"<<","<<"TF_allin1_LLEstimation_muon_newTFbins.root"<<","<<"TMVAClassification_BDT_100trees_2maxdepth.weights.xml"<<endl
