...
// /int line_color[n_pl+1] = {kBlack, kRed, kGreen+2, kBlue, kRed};                                                                               
int line_color[10] = {kBlack,kGray+2,kBlue,kViolet+2,kBlue+2,kCyan+2,kGreen-2,kOrange+1,kMagenta, kPink-9};//{9,kCyan+2,45,kMagenta,kGray+1,kRed,kBlue+2,kMagenta,kCyan};
...
void generate_1Dplot(vector<TH2D*> hist, char const *tag_name="",char const *xlabel="",char const *ylabel="", float energy=-1, int rebin=-1,double ymin=0,double ymax=0,int xmin=-1,int xmax=-1,
		     bool normalize=false, bool log_flag=true, bool DoRebin=false, bool save_canvas=true,  char const * legend_texts="", char const *legend_title="", TString model=""){  
  ...
  canvas_n1->SetRightMargin(0.12);//0.035
  ...
  legend = new TLegend(0.6,0.71,0.94,0.945);  
  legend->SetTextSize(0.025);
  legend->SetLineColor(kWhite);
  //legend->SetNColumns(3);
  char* lhead = new char[100];
  cout<<"before legend fixing "<<endl; 
 //  sprintf(lhead,"#bf{%s} ",legend_title);
  ...
  for(int i =0;i<(int)hist.size(); i ++) {
    ...
    if(normalize) {
     ...
     hist.at(i)->GetZaxis()->SetLabelSize(0.04);
    ...
    hist.at(i)->GetYaxis()->SetLabelSize(0.05);//x_label_size);
    hist.at(i)->SetLineColor(line_color[i]);
    hist.at(i)->SetTitle(" ");
    //
    hist.at(i)->GetXaxis()->SetTitleSize(0.05);
  ...
  //  hist.at(i)->GetXaxis()->SetRange(1, hist.at(i)->GetNbinsX() + 1);
    ...
    cout<<legend_texts<<endl;
    leg_entry[i] = legend->AddEntry(hist.at(i),legend_texts,"l");
  ...
  for(int i = 0;i<(int)hist.size(); i++) {
    // if(!normalize) {
    //   if(model.Contains("TChiWG") || model.Contains("T6ttZg") || model.Contains("TChiNG"))
    // 	hist.at(i)->GetYaxis()->SetRangeUser(0.01,1000*ymax);
    //   else if(model.Contains("WGJets") || model.Contains("WlnuJets") || model.Contains("ttbarG")|| model.Contains("ttbarJets") || model.Contains("GJets") || model.Contains("ZnunuGJets") ) hist.at(i)->GetYaxis()->SetRangeUser(0.01,100000*ymax);
...
//     hs_var->GetYaxis()->SetTitleOffset(1.0);
  ...
  textOnTop->DrawLatexNDC(0.7,0.965,en_lat);
...
void two2D_kinematic(string pathname, string model, string gluino_m, string baseline1)
{
  ...
  vector<string>baseline2, baseline;
  ...
  baseline = {"overlap_removal","MET_200","MET_300","MET_300_phopt_100","MET_200_phopt_100"};//{"No_selection","veto_chargTracks","photon_pT40","MET_100","nJets_2","ST_300","dPhi_cut","overlap_removal","MET_200","MET_300"}; //lepton_veto
  vector <string > varName ={"h_ST_vs_EMObjPt","h_Emobj_PtvsEta","h_Emobj_PtvsPhi","h_Emobj_EtavsPhi","h_nJetsvsBjets","h_nJetsvsMET","h_nbJetsvsMET","h_METvsMETPhi","h_phopTvsNjets","h_phopTvsNbjets","h_phopTvsMtphoMET","h_METvsPhopT","h_STvsNjets","h_STvsNbjets","h_STvsMET"};//"h_St","h_HT","h_NhadJets","h_NBJets","h_MET","h_PhoPt","h_Photon_Eta","h_Photon_Phi","h_MET_Phi","FR_nbtagBins","h_Mt_phoMET","h_dPhi_phoMet","h_Nphotons","h_dPhi_METLeadJet"};
  vector <string> xlabel = {"p_{T}^{#gamma} [GeV]","p_{T}^{#gamma} [GeV]","p_{T}^{#gamma} [GeV]","#eta^{#gamma}","N_{jets}","p_{T}^{miss} [GeV]","p_{T}^{miss} [GeV]","p_{T}^{miss} [GeV]","p_{T}^{#gamma} [GeV]","p_{T}^{#gamma} [GeV]","p_{T}^{#gamma} [GeV]","p_{T}^{miss} [GeV]","Sum of p_{T}^{Jets} & p_{T}^{#gamma} [GeV]","Sum of p_{T}^{Jets} & p_{T}^{#gamma} [GeV]","Sum of p_{T}^{Jets} & p_{T}^{#gamma} [GeV]"};//"Sum of p_{T}^{Jets} & p_{T}^{#gamma} [GeV]","HT[GeV]","N_{jets}","N_{ b-jets}","p_{T}^{miss} [GeV]","p_{T}^{#gamma} [GeV]","#eta^{#gamma}","#phi^{#gamma}","#phi^{MET}","Bin No.","M_{T}^{miss & #gamma} [GeV]","dPhi(#gamma,MET)","N_{#gamma}","d#phi(p_{T}^{miss},lead Jet1)"};
  vector <string> ylabel = {"Sum of p_{T}^{Jets} & p_{T}^{#gamma} [GeV]","#eta^{#gamma}","#phi^{#gamma}","#phi^{#gamma}","N_{ b-jets}","N_{jets}","N_{ b-jets}","#phi^{MET}","N_{jets}","N_{ b-jets}","M_{T}^{miss & #gamma} [GeV]","p_{T}^{#gamma} [GeV]","N_{jets}","N_{ b-jets}","p_{T}^{miss}"};
  ...
  //{"No selection","veto lept+isoTracks","p_{T}^{#gamma} > 40","p_{T}^{miss} [GeV] > 100","N_{jets} #geq 2","Sum of p_{T}^{Jets} & p_{T}^{#gamma} > 300 ","d#phi(p_{T}^{miss},lead Jets) > 0.3 ","Preselection","p_{T}^{miss} [GeV] > 200","p_{T}^{miss} [GeV] > 300"};

  vector <string>legend_title;
  vector<string> filetag= {"FullRun2"};// {"2018","2017","2016postVFP","2016preVFP","2016","FullRun2"};
  vector<float> energyy={137.19,137.19,137.19,137.19,137.19,137.19,137.19,137.19,137.19};//{ 59.74,41.53,16.5,19.5,36,137.19};
  vector <int> rebinX, rebinY;
  rebinX={5,4,4,4,1,5,5,5,4,4,4,5,5,5,5};
  rebinY ={4,4,4,4,1,1,1,4,1,1,4,4,1,1,4};
  vector<double> ymin ={200,-5,-5,-5,0,0,0,-5,0,0,0,0,0,0,100,};
  vector<double> ymax={2500,5,5,5,14,20,14,5,20,14,700,800,20,20,1000};
  vector<double> xmin ={0,0,0,-5,0,0,0,0,0,0,0,0,200,200,200};
  vector<double> xmax={800,800,800,5,16,1000,1000,1000,800,800,800,1000,3000,3000,3000};

  cout<<"different vector sizes "<<endl;
  cout<<varName.size()<<"\t"<<baseline.size()<<"\t"<<xlabel.size()<<"\t"<<rebinX.size()<<"\t"<<rebinY.size()<<"\t"<<xmax.size()<<"\t"<<xmin.size()<<"\t"<<legend_texts.size()<<endl;
  bool flag=false;
n_files=7;
  if(Model.Contains("T5bbbbZg") && Gluino_m.Contains("2200")){
    Nlsp_m = {"10","50","100","200","400","600","1000"};
    legend_title={"(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 50 GeV)","(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 100 GeV)","(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)","(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)","(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)","(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    Nlsp_m = {"10","50","100","200","400","600","1000"};
        legend_title={"(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 50 GeV)","(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 100 GeV)","(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)","(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)","(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)","(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    Nlsp_m = {"10","50","100","200","400","600","1000"};
        legend_title={"(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 50 GeV)","(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 100 GeV)","(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)","(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)","(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)","(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    Nlsp_m = {"10","50","100","200","400","600","1000"};
        legend_title={"(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 50 GeV)","(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 100 GeV)","(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)","(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)","(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)","(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[6] = new TFile("out_T5bbbbZg_2500_1000.root");
     ...
     n_files=6;
     legend_title={"(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0} = 127 GeV)", "(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0} = 150 GeV)", "(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)","(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)","(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)","(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[5] = new TFile("out_T5qqqqHg_2500_1000.root");
     ...
     Nlsp_m = {"127","150","200","400","600","1000"};
     n_files=6;
     legend_title={"(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 127 GeV)", "(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 150 GeV)", "(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)","(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)","(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)","(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[5] = new TFile("out_T5qqqqHg_2400_1000.root");
     ...
     Nlsp_m = {"127","150","200","400","600","1000"};
     n_files=6;
     legend_title={"(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 127 GeV)", "(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 150 GeV)", "(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)","(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)","(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)","(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[5] = new TFile("out_T5qqqqHg_2300_1000.root");
     ...
     Nlsp_m = {"127","150","200","400","600","1000"};
     n_files=6;
     legend_title={"(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0} = 127 GeV)", "(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 150 GeV)", "(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)","(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)","(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)","(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[5] = new TFile("out_T5qqqqHg_2200_1000.root");
     ...
     Nlsp_m = {"10","50","100","200","400","600","1000"};
     n_files=7;
     legend_title={"(m_{#tilde{g}} = 2100 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2100 GeV, m_{#tilde {#chi}_{1}^{0}} = 50 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2100 GeV, m_{#tilde {#chi}_{1}^{0}} = 100 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2100 GeV, m_{#tilde {#chi}_{1}^{0}} = 200 GeV)" ,"#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2100 GeV, m_{#tilde {#chi}_{1}^{0}} = 400 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2100 GeV, m_{#tilde {#chi}_{1}^{0}} = 600 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2100 GeV, m_{#tilde {#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[6] = new TFile("out_T5ttttZg_2100_1000.root");    
     ...
     Nlsp_m = {"10","50","100","200","400","600","1000"};
     n_files=7;
     legend_title={"(m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2200 GeV, m_{#tilde {#chi}_{1}^{0}} = 50 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2200 GeV, m_{#tilde {#chi}_{1}^{0}} = 100 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2200 GeV, m_{#tilde {#chi}_{1}^{0}} = 200 GeV)" ,"#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2200 GeV, m_{#tilde {#chi}_{1}^{0}} = 400 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2200 GeV, m_{#tilde {#chi}_{1}^{0}} = 600 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2200 GeV, m_{#tilde {#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[6] = new TFile("out_T5ttttZg_2200_1000.root");    
     ...
     Nlsp_m = {"10","50","100","200","400","600","1000"};
     n_files=7;
     legend_title={"(m_{#tilde{g}} = 2300 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2300 GeV, m_{#tilde {#chi}_{1}^{0}} = 50 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2300 GeV, m_{#tilde {#chi}_{1}^{0}} = 100 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2300 GeV, m_{#tilde {#chi}_{1}^{0}} = 200 GeV)" ,"#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2300 GeV, m_{#tilde {#chi}_{1}^{0}} = 400 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2300 GeV, m_{#tilde {#chi}_{1}^{0}} = 600 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2300 GeV, m_{#tilde {#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[6] = new TFile("out_T5ttttZg_2300_1000.root");    
     ...
     Nlsp_m = {"10","50","100","200","400","600","1000"};
     n_files=7;
     legend_title={"(m_{#tilde{g}} = 2400 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2400 GeV, m_{#tilde {#chi}_{1}^{0}} = 50 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2400 GeV, m_{#tilde {#chi}_{1}^{0}} = 100 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2400 GeV, m_{#tilde {#chi}_{1}^{0}} = 200 GeV)" ,"#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2400 GeV, m_{#tilde {#chi}_{1}^{0}} = 400 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2400 GeV, m_{#tilde {#chi}_{1}^{0}} = 600 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2400 GeV, m_{#tilde {#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[6] = new TFile("out_T5ttttZg_2400_1000.root");    
     ...
     Nlsp_m = {"10","50","100","200","400","600","1000"};
     n_files=7;
     legend_title={"(m_{#tilde{g}} = 2500 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2500 GeV, m_{#tilde {#chi}_{1}^{0}} = 50 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2500 GeV, m_{#tilde {#chi}_{1}^{0}} = 100 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2500 GeV, m_{#tilde {#chi}_{1}^{0}} = 200 GeV)" ,"#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2500 GeV, m_{#tilde {#chi}_{1}^{0}} = 400 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2500 GeV, m_{#tilde {#chi}_{1}^{0}} = 600 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2500 GeV, m_{#tilde {#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[6] = new TFile("out_T5ttttZg_2500_1000.root");    
     ...
     Nlsp_m = {"10","50","100","200","400","600","1000"};
     n_files=7;
     legend_title={"(m_{#tilde{g}} = 2600 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2600 GeV, m_{#tilde {#chi}_{1}^{0}} = 50 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2600 GeV, m_{#tilde {#chi}_{1}^{0}} = 100 GeV)", "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2600 GeV, m_{#tilde {#chi}_{1}^{0}} = 200 GeV)" ,"#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2600 GeV, m_{#tilde {#chi}_{1}^{0}} = 400 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2600 GeV, m_{#tilde {#chi}_{1}^{0}} = 600 GeV)","#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2600 GeV, m_{#tilde {#chi}_{1}^{0}} = 1000 GeV)"};
    ...
    f[6] = new TFile("out_T5ttttZg_2600_1000.root");    
     ...
     n_files=6;
       legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1300 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1300 GeV, m_{#tilde{#chi}_{1}^{0}} = 50 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1300 GeV, m_{#tilde{#chi}_{1}^{0}} = 100 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1300 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1300 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)","#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1300 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)"};
     ...
     Nlsp_m = {"10","50","100","200","400","600"};
     n_files=6;
     legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1400 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1400 GeV, m_{#tilde{#chi}_{1}^{0}} = 50 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1400 GeV, m_{#tilde{#chi}_{1}^{0}} = 100 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1400 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)", " #tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1400 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1400 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)"};
    ...
    f[5] = new TFile("out_T6ttZg_1400_600.root");
     ...
     Nlsp_m = {"10","50","100","200","400","600"};
     n_files=6;
     legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1600 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1600 GeV, m_{#tilde{#chi}_{1}^{0}} = 50 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1600 GeV, m_{#tilde{#chi}_{1}^{0}} = 100 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1600 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)", " #tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1600 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1600 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)"};
    ...
    f[5] = new TFile("out_T6ttZg_1600_600.root");
     ...
     Nlsp_m = {"10","50","100","200","400","600"};
     n_files=6;
     legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1800 GeV, m_{#tilde{#chi}_{1}^{0}} = 10 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1800 GeV, m_{#tilde{#chi}_{1}^{0}} = 50 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1800 GeV, m_{#tilde{#chi}_{1}^{0}} = 100 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1800 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)", " #tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1800 GeV, m_{#tilde{#chi}_{1}^{0}} = 400 GeV)", "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{t}} = 1800 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)"};
  ...
  //electroweakino production
    ...
    Nlsp_m =  {"2018","2017","2016postVFP","2016preVFP","FullRun2"}; 
    energyy = { 59.74,41.53,16.5,19.5,36,137.19}; 
    n_files=5;
  ...
  int   n_cut =1;//baseline.size();
  for(int i_file=0; i_file<n_files;i_file++)
    {      
      for(int i_var=0; i_var<n_var;i_var++)
	{
	  // vector<TH2D*> hist_list_Njets;
	  // vector<TH2D*> hist_list_Bjets;
	  for(int i_cut=0; i_cut<n_cut;i_cut++){
	    vector<TH2D*> hist_list_Njets;
	    vector<TH2D*> hist_list_Bjets;
	    if (varName[i_var].find("Phi") != std::string::npos) continue;
	    if (varName[i_var].find("Eta") != std::string::npos) continue;
	    if (varName[i_var].find("h_nJetsvsBjets") != std::string::npos) continue;
	    if (varName[i_var].find("h_phopTvsMtphoMET") != std::string::npos) continue;

	    sprintf(hist_name,"%s_%s",varName[i_var].c_str(),baseline1.c_str());
	    cout<<hist_name<<"\t"<<i_cut<<"\t"<<i_var<<"\t"<<i_file<<"\t"<<f[i_file]->GetName()<<endl;
	    TH2D* h_resp = (TH2D*)f[i_file]->Get(hist_name);
	    cout<<"resp "<<h_resp->Integral()<<"\t"<<rebinX[i_var]<<"\t"<<rebinY[i_var]<<"\t"<<xmin[i_var]<<"\t"<<xmax[i_var]<<endl;
	    // h_resp->Rebin(rebin[i_var]);
	    // if(rebin[i_var]!=1){
	    //   h_resp->Rebin(2);
	    // }
	    h_resp->GetXaxis()->SetRangeUser(xmin[i_var],xmax[i_var]);// setMyRange(h_resp,xmin[i_var],xmax[i_var]+0.01*xmax[i_var]);
	    h_resp->GetYaxis()->SetRangeUser(ymin[i_var],ymax[i_var]);
	    //setLastBinAsOverFlow(h_resp);
	    h_resp->RebinX(rebinX[i_var]);
	    h_resp->RebinY(rebinY[i_var]);
            if(rebinX[i_var]!=1){
              h_resp->RebinX(2);
	    }
	    if(rebinY[i_var]!=1){
	  ...
	  int xrange=0.0;
	  sprintf(full_path,"%s/%s_%s_%s_%s_%s",pathname.c_str(),varName[i_var].c_str(),model.c_str(),gluino_m.c_str(),Nlsp_m[i_file].c_str(),baseline1.c_str());
	  cout<<"varName "<< varName[i_var].c_str() <<"\t"<<Nlsp_m[i_file].c_str()<<"\t"<<i_var<<"\t"<<i_file<<"\t"<<xlabel[i_var].c_str()<<"\t"<<rebinX[i_var]<<"\t"<<ymin[i_var]<<"\t"<<ymax[i_var]<<"\t"<<xmin[i_var]<<"\t"<<xmax[i_var]<<"\t"<<legend_title[i_file].c_str()<<endl;
	  if(i_var==2 || i_var==3)
	    generate_1Dplot(hist_list_Njets,full_path,xlabel[i_var].c_str(),ylabel[i_var].c_str(),energy,rebinX[i_var],ymin[i_var],ymax[i_var],xmin[i_var],xmax[i_var],false,true,false,true,legend_texts[i_cut].c_str(),legend_title[i_file].c_str(), Model);
	  else
	    generate_1Dplot(hist_list_Njets,full_path,xlabel[i_var].c_str(),ylabel[i_var].c_str(),energy,rebinX[i_var],ymin[i_var],ymax[i_var],xmin[i_var],xmax[i_var],false,true,false,true,legend_texts[i_cut].c_str(),legend_title[i_file].c_str(), Model);
      ...
      // 	  // hist_list_Njets.at(0)->GetXaxis()->SetLabelSize(0.045);
      // 	  // hist_list_Njets.at(1)->GetXaxis()->SetLabelSize(0.045);
      // 	  //	  TH2D* h_temp = (TH2D*)hist_list_Bjets.at(0)->Clone();	  
      ...
      // 	  //	  TH2D*	h_temp1 = (TH2D*)hist_list_Bjets.at(1)->Clone();
      ...
      // 	  hNjets_ratio =(TH2D*)hist_list_Njets.at(1)->Clone();//(TH2D*)hist_list_Bjets.at(1)->Clone();
