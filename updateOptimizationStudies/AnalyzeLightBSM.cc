...
void AnalyzeLightBSM::EventLoop(const char *data,const char *inputFileList, const char *sample , const char *outFileName,  const char* phoID) {
 ...
 char* s_cross = new char[100];
  ...
  //setting up for BDT
  float met=0.0,st=0.0, njets=0, btags=0,mTPhoMet=0.0,dPhi_PhoMet=0.0,dPhi_MetJet1=0.0,dPhi_MetJet2=0.0,PhoPt=0.0,dPhi_Pho_Jet2=0.0,dPhi_Pho_Jet1=0.0,LeadJets1Pt=0.0,LeadJets2Pt=0.0;
 ...
 // //   }
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)
	...
	cout<<"===load tree entry ==="<<jentry<<endl;
      ...
      if(s_sample.Contains("TChiNG")) wt = Weight*lumiInfb;
...
// // ********* This is to account into all visible energy: Adding photon matching with Jet ******************//
    if(Debug)
      cout<< "Entry: "<<jentry<<"\t"<<"before adding ST "<<"\t"<<nPhotons<<endl;
    ...
    if(Debug)
      cout<< "Entry: "<<jentry<<"\t"<<bestPhoton.Phi()<<endl;
     TLorentzVector Met;
    ...
    //    h_madminPhotonDeltaR_preSelection->Fill(madMinPhotonDeltaR,wt); 
	 ...
	 // searchBin = getBinNoV6_WithOnlyBLSelec_v5(bestPhoton.Pt(),ST);
	 if(jentry<1000)
	   {	   searchBin = getBinNoV8_ST_phoPT_Njetsbjets_MET(ST,bestPhoton.Pt(),nHadJets,BTags,MET);
	     cout<<jentry<<"\t"<<"search bin "<<searchBin<<"\t"<<bestPhoton.Pt()<<"\t"<<ST<<"\t"<<BTags<<endl;}
	 // if(bestPhoton.Pt()>=400){
	 //   searchBin = getBinNoV6_WithOnlyBLSelec_v5(bestPhoton.Pt(),ST);
	 //   cout<<jentry<<"\t"<<"search bin "<<searchBin<<"\t"<<bestPhoton.Pt()<<"\t"<<ST<<endl;}
	 FillHistogram_Kinematics(1,nHadJets,BTags, bestPhoton.Pt(), mTPhoMET, dPhi_PhoMET, ST, wt, nPhotons, bestPhoton.Eta(),bestPhoton.Phi(),dPhi_METjet1,mvaValue);
	 if(mvaValue>0.0)
	   FillHistogram_Kinematics(6,nHadJets,BTags, bestPhoton.Pt(), mTPhoMET, dPhi_PhoMET, ST, wt, nPhotons, bestPhoton.Eta(),bestPhoton.Phi(),dPhi_METjet1,mvaValue);
      if(Debug)
	...
	FillHistogram_Kinematics(5,nHadJets,BTags, bestPhoton.Pt(), mTPhoMET, dPhi_PhoMET, ST, wt, nPhotons, bestPhoton.Eta(),bestPhoton.Phi(),dPhi_METjet1,mvaValue);
	if(mvaValue>0.0)
	  FillHistogram_Kinematics(10,nHadJets,BTags, bestPhoton.Pt(), mTPhoMET, dPhi_PhoMET, ST, wt, nPhotons, bestPhoton.Eta(),bestPhoton.Phi(),dPhi_METjet1,mvaValue);
      }
      if(Debug)
...
int AnalyzeLightBSM::getBinNo_withBDTScore(double mvaValue){

  int sBin=0,m_i=0;
  //if(nHadJets>=2 && nHadJets<=4)     { sBin=0;}
  for(int i=0;i<BDTscore_bins.size()-1;i++){
      if(BDTscore_bins[i]<-2) continue;
      int sBin1=sBin;
      m_i++;
      if(mvaValue >= BDTscore_bins[i] && mvaValue < BDTscore_bins[i+1]){ sBin = sBin+m_i;
        break; }
      else if(mvaValue >= BDTscore_bins[BDTscore_bins.size()-1])  { sBin = 10 ;
        break; }
  
  }
  return sBin;
}
int AnalyzeLightBSM::getBinNoV4_ST_Njets_bjets(int nHadJets,int nbjets, double ST)
{
    int sBin=-100,m_i=0;
  if(nbjets==0 ){
    if(nHadJets>=2 && nHadJets<=4)     { sBin=0;}
    else if(nHadJets==5 || nHadJets==6){ sBin=5;}
    else if(nHadJets>=7)               { sBin=10;}
  }
  else{
    if(nHadJets>=2 && nHadJets<=4)     { sBin=15;}
    else if(nHadJets==5 || nHadJets==6){ sBin=20;}
    else if(nHadJets>=7)               { sBin=25;}
  }
  for(int i=0;i<ST_bins.size()-1;i++){
    //if(ST_bins[i]<299.99) continue;
    m_i++;
    if(ST >= ST_bins[i] && ST < ST_bins[i+1]){ sBin = sBin+m_i;break; }
    else if(ST >= ST_bins[ST_bins.size()-1])  { sBin = sBin+5; break; }
  }
  
  return sBin;
}

int AnalyzeLightBSM::getBinNoV5_ST_Njets_bjets_MET(int nHadJets,int nbjets, double ST, double Met)
{
    int sBin=-100,m_i=0;
    if(Met<=400){
      if(nbjets==0 ){
	...
	if(ST >= ST_bins[i] && ST < ST_bins[i+1]){ sBin = sBin+m_i;break; }
     ...
     if(nbjets==0 ){
        if(nHadJets>=2 && nHadJets<=4)     { sBin=30;}
        else if(nHadJets==5 || nHadJets==6){ sBin=35;}
        else if(nHadJets>=7)               { sBin=40;}
      }
      else{
        if(nHadJets>=2 && nHadJets<=4)     { sBin=45;}
        else if(nHadJets==5 || nHadJets==6){ sBin=50;}
        else if(nHadJets>=7)               { sBin=55;}
      }
      for(int i=0;i<ST_bins.size()-1;i++){
        m_i++;
        if(ST >= ST_bins[i] && ST < ST_bins[i+1]){ sBin = sBin+m_i;break; }
...
int AnalyzeLightBSM::getBinNoV6_ST_Njets_bjets_PhopT(int nHadJets,int nbjets, double ST, double pho_pt)
{
    int sBin=-100,m_i=0;
    if(pho_pt<=100){
      if(nbjets==0 ){
...
int AnalyzeLightBSM::getBinNoV1_ST_phopt_bjets(double pho_pt, double ST, int bjets){

  int sBin=0, m_i=1,sBin1=0,n_i=0;
  if(bjets==0){
   for(int i=0;i<phoPt_bins.size()-1;i++){
    if(i!=0)
      m_i++;
    if(pho_pt >= phoPt_bins[i] && pho_pt < phoPt_bins[i+1])
      {
        sBin = sBin+((m_i-1)*5);
        break;
      }
    else if(pho_pt >= phoPt_bins[phoPt_bins.size()-1])
      {
        sBin = 20;
        break;
      }
   ...
   //   cout<<"sBin "<<sBin<<"\t"<<m_i<<"\t"<<pho_pt<<"\t"<<ST<<"\t"<<phoPt_bins.size()<<"\t"<<phoPt_bins[phoPt_bins.size()-1]<<"\t"<< phoPt_bins[0]<<"\t"<<sBin%5<<endl;                                                 
   if(sBin%5==0)
     {
       // cout<<"sBin1 in loop "<<sBin<<"\t"<<m_i<<"\t"<<pho_pt<<"\t"<<ST<<"\t"<<endl;
       for(int i=0;i<ST_bins.size()-1;i++){
        n_i++;
    ...
    m_i=6;
    //cout<<"checks "<<m_i<<endl;
    for(int i=0;i<phoPt_bins.size()-1;i++){
      if(i!=0)
	m_i++;
      //cout<<"checks1 "<<m_i<<endl;
      if(pho_pt >= phoPt_bins[i] && pho_pt < phoPt_bins[i+1])
	{
	  sBin = sBin+((m_i-1)*5);
	  break;
      ...
      else if(pho_pt >= phoPt_bins[phoPt_bins.size()-1])
	{
      ...
      //cout<<"checks2 "<<m_i<<endl;
    }
    //    cout<<"sBin b>0 loop "<<"\t"<<bjets<<"\t"<<sBin<<"\t"<<m_i<<"\t"<<n_i<<"\t"<<pho_pt<<"\t"<<ST<<"\t"<<phoPt_bins.size()<<"\t"<<phoPt_bins[phoPt_bins.size()-1]<<"\t"<< phoPt_bins[0]<<"\t"<<sBin%5<<endl;
    
    if(sBin%5==0)
      {
	//cout<<"sBin1 in loop "<<sBin<<"\t"<<m_i<<"\t"<<n_i<<"\t"<<pho_pt<<"\t"<<ST<<"\t"<<endl;
	for(int i=0;i<ST_bins.size()-1;i++){
	  n_i++;
	  // cout<<"sBin1 in loop "<<sBin<<"\t"<<m_i<<"\t"<<n_i<<"\t"<<pho_pt<<"\t"<<ST<<"\t"<<endl;
	  if(ST>=ST_bins[i] && ST<ST_bins[i+1]) {sBin1=sBin+n_i; break;}
...
int AnalyzeLightBSM::getBinNoV8_ST_phoPT_Njetsbjets_MET(double ST, double pho_pt, int nHadJets, int nbjets,double Met){
  int sBin=-100,m_i=0;
  cout<<"inside the bin "<<endl;
if(pho_pt<=100){
  if(ST>300 && ST<=1500){
  if(nbjets==0 ){
    if(nHadJets>=2 && nHadJets<=4)     { sBin=0;}
  ...
  if(sBin==0){
    ...
    {
      if(nbjets==0 ){
        if(nHadJets>=2 && nHadJets<=4)     { sBin=33;}
        else if(nHadJets==5 || nHadJets==6){ sBin=40;}
        else if(nHadJets>=7)               { sBin=46;}
      }
      else{
        if(nHadJets>=2 && nHadJets<=4)     { sBin=51;}
        else if(nHadJets==5 || nHadJets==6){ sBin=56;}
        else if(nHadJets>=7)               { sBin=61;}
      }
      ...
      if(sBin==33){
        for(int i=0;i<METLowEdge_v3.size()-1;i++){
          ...
          if(MET >= METLowEdge_v3[i] && MET < METLowEdge_v3[i+1]){ sBin = sBin+m_i;
            break; }
          else if(MET >= METLowEdge_v3[METLowEdge_v3.size()-1])  { sBin = 40         ;
            break; }
        }
      }

  else if(sBin==40 ){//|| sBin==33 || sBin==39){                                                                                                                                                                                                                                                                                                                                                                                                                          
    int sBin1=sBin;
    for(int i=0;i<METLowEdge_v3_1.size()-1;i++){
      if(METLowEdge_v3_1[i]<199.99) continue;
      m_i++;
  ...
  if(ST>300 && ST<=1500){
  if(nbjets==0 ){
    if(nHadJets>=2 && nHadJets<=4)     { sBin=66;}
    else if(nHadJets==5 || nHadJets==6){ sBin=73;}
    else if(nHadJets>=7)               { sBin=79;}
  }
  else{
    if(nHadJets>=2 && nHadJets<=4)     { sBin=84;}
    else if(nHadJets==5 || nHadJets==6){ sBin=89;}
    else if(nHadJets>=7)               { sBin=94;}
  }
  if(sBin==66){
    for(int i=0;i<METLowEdge_v3.size()-1;i++){
      if(METLowEdge_v3[i]<199.99) continue;
      ...
      if(MET >= METLowEdge_v3[i] && MET < METLowEdge_v3[i+1]){ sBin = sBin+m_i;
        break; }
      else if(MET >= METLowEdge_v3[METLowEdge_v3.size()-1])  { sBin = 73         ;
        break; }
    }
  }
  else if(sBin==73 ){
    ...
    {
      if(nbjets==0 ){
        if(nHadJets>=2 && nHadJets<=4)     { sBin=99;}
        else if(nHadJets==5 || nHadJets==6){ sBin=106;}
        else if(nHadJets>=7)               { sBin=112;}
      }
      else{
        if(nHadJets>=2 && nHadJets<=4)     { sBin=117;}
        else if(nHadJets==5 || nHadJets==6){ sBin=122;}
        else if(nHadJets>=7)               { sBin=127;}
      }
      ...
      if(sBin==99){
        for(int i=0;i<METLowEdge_v3.size()-1;i++){
          ...
          if(MET >= METLowEdge_v3[i] && MET < METLowEdge_v3[i+1]){ sBin = sBin+m_i;
            break; }
          else if(MET >= METLowEdge_v3[METLowEdge_v3.size()-1])  { sBin = 106         ;
            break; }
        }
      }

  else if(sBin==106 ){//|| sBin==33 || sBin==39){                                                                                                                                                                                                                                                                                                                                                                                                                          
    int sBin1=sBin;
    for(int i=0;i<METLowEdge_v3_1.size()-1;i++){
      if(METLowEdge_v3_1[i]<199.99) continue;
      m_i++;
...
int AnalyzeLightBSM::getBinNoV2_ST_MET(double Met, double ST){
  ...
  int sBin=0, m_i=1,sBin1=0,n_i=0;
   ...
   //  cout<<"sBin "<<sBin<<"\t"<<m_i<<"\t"<<pho_pt<<"\t"<<ST<<"\t"<<phoPt_bins.size()<<"\t"<<phoPt_bins[phoPt_bins.size()-1]<<"\t"<< phoPt_bins[0]<<endl;
   if(sBin%5==0)
    {
      for(int i=0;i<ST_bins.size()-1;i++){
        n_i++;
...
int AnalyzeLightBSM::getBinNoV6_WithOnlyBLSelec_v5(double pho_pt, double ST){
...
int AnalyzeLightBSM::getBinNoV6_WithOnlyBLSelec_v4(int nHadJets,int nbjets,double ST)
{
  int sBin=-100,m_i=0;
...
int AnalyzeLightBSM::getBinNoV6_WithOnlyBLSelec_v3(int nHadJets,int nbjets,double phopt)
{
  int sBin=-100,m_i=0;
  if(phopt>40 && phopt<=100){
  if(nbjets==0 ){
    if(nHadJets>=2 && nHadJets<=4)     { sBin=0;}
    else if(nHadJets==5 || nHadJets==6){ sBin=7;}
    else if(nHadJets>=7)               { sBin=13;}
...
void AnalyzeLightBSM::FillHistogram_Kinematics(int i, int Njets, int btags, double pho_Pt, double mt_phoMET, double dPhi, double ST, double wt, int nphotons, double pho_eta, double pho_phi, double dphi_METJets, double mvaValue ){
  ...
  searchBin = getBinNoV6_WithOnlyBLSelec_v3(Njets,btags,pho_Pt);
  h_Sbins_LL_v3[i]->Fill(searchBin,wt);
  searchBin = getBinNo_withBDTScore(mvaValue);
  h_Sbins_LL_v4_bdtscore[i]->Fill(searchBin,wt);
  searchBin = getBinNoV6_WithOnlyBLSelec_v4(Njets,btags,ST);
  h_Sbins_LL_v4[i]->Fill(searchBin,wt);
  searchBin = getBinNoV6_WithOnlyBLSelec_v5(pho_Pt,ST);
  h_Sbins_LL_v5[i]->Fill(searchBin,wt);

  searchBin = getBinNoV1_ST_phopt_bjets(pho_Pt,ST,btags);
  ...
  h_Sbins_LL_newSbins_v4[i]->Fill(searchBin,wt);

  searchBin = getBinNoV5_ST_Njets_bjets_MET(Njets,btags,ST,MET);
