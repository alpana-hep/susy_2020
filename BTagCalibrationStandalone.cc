...
throw std::exception();
  ...
  for (unsigned int i = 0; i < strlen(chars); ++i) {
    //    vec[0].erase(remove(vec[0].begin(),vec[0].end(),chars[i]),vec[0].end());
...
{
  //  cout<<"are we in this add entry "<<&entry<<endl;
...
{
  std::string line;
  //cout<<"starting reading the csv file"<<endl;
  // firstline might be the header
  getline(s,line);
  //  cout<< "printing line "<<line<<"\t"<<std::string::npos<<endl;
  ...
  while (getline(s,line)) {
    //cout<<"are we in this loop"<<endl;
    line = BTagEntryS::trimStr(line);
    //cout<<"are we after triming it "<<line<<endl;
    if (line.empty()) {  // skip empty lines
      continue;
    }
    //cout<<"are we adding something"<<endl;
    addEntry(BTagEntryS(line));
  }
}
