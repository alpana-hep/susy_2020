...
// /int line_color[n_pl+1] = {kBlack, kRed, kGreen+2, kBlue, kRed};                                                                               
int line_color[12] = {kGray+1,kOrange-3,kCyan+1,kYellow+2,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2};//{9,kCyan+2,45,kMagenta,kGray+1,kRed,kBlue+2,kMagenta,kCyan};
int line_color1[12]= {kGray+1,kOrange-3,kCyan+1,kYellow+2,kBlue,kGreen+2,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2};
int line_color2[12] = {kGray+1,kViolet,kBlue,kGreen+2,kGray,kViolet+2,kGreen-2,kYellow+1,kGray+2};//,kMagenta};
...
void decorate(TH1F* hist,int i, int j){
  ...
  //   hist->SetFillColor(col[i]);
   ...
   vector<int> Style;
   if(j==1){
     col={kGray+1,kOrange-3,kCyan+1,kYellow+2,kViolet,kGreen+2,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
     Style= {1001,1001,1001,1001};

   }
   else if(j==2){
     col={kGray+1,kOrange-3,kCyan+1,kYellow+2,kBlue,kGreen+2,kBlue,kGreen+2,kGray+1,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
     Style={1001,1001,1001,1001};
   }
   else if(j==3){
...
//     Tl.SetTextSize(0.055);
  ...
  gPad->Modified();
    ...
    if(which_Lept==2)
      hist_ratio->GetYaxis()->SetRangeUser(-0.5,2.0);
    else
      hist_ratio->GetYaxis()->SetRangeUser(-0.5,2.0);
    if(which_TFbins==1) //default 8 bins                                                                                                    
...
void EveCateg_LL_ratio(string pathname, int which_Lept, int which_TFBins, bool all)
{
...
char* hname2 = new char[200];
  ...
  if(which_Lept==1){
    ...
    baseline1={"Elec_CR","FailAcep_ElecSR","FailIso_ElecSR","FailId_ElecSR"};//"Elec_SR"};//,"TauHad_SR","Mu_SR","Elec_SR","FailAcep_ElecSR","FailId_ElecSR","FailIso_ElecSR","Elec_SR","Elec_"};//
    legend_texts ={"(1e,1#gamma) CR","Fail Accep","Fail Iso","Fail Id"};//,"(0e,1#gamma) SR"};//,"#tau-had SR","lost #mu SR","lost e SR","(1l,1#gamma) CR","Failed Iso"};
  ...
  if(which_Lept==2){
    ...
    sprintf(hname,"%s_phoID_loose_09Jan24",string_png);
    baseline1 = {"Mu_CR","FailAcep_MuSR","FailIso_MuSR","FailId_MuSR"};//"Mu_SR","TauHad_SR"};
    legend_texts ={"(1#mu,1#gamma) CR","Fail Accep","Fail Iso","Fail Id"};//"(0#mu,1#gamma) SR","(#tau-had, 1#gamma) SR"};//,"(1#mu,1#gamma) CR"};//,"(#tau-had, 1#gamma) SR"};
  }
  if(which_Lept==3){
  ...
  for(int i_file=0; i_file<n_files;i_file++)
      ...
      for(int i_cut=0; i_cut<baseline1.size();i_cut++)
	{
      ...
      cout<<" hist_list_Njets.size() "<<hist_list_Njets.size()<<"\t hist_list_Bjets.size() "<<hist_list_Bjets.size()<<endl;
       for (int j =2; j<10;j++)
        {
	  cout<<"\t"<<hist_list_Njets.at(0)->GetBinContent(j)<<"\t"<<hist_list_Njets.at(1)->GetBinContent(j)<<"\t"<<hist_list_Njets.at(2)->GetBinContent(j)<<"\t"<<hist_list_Njets.at(3)->GetBinContent(j)<<endl;
	}
      ...
      hNjets_ratio->Add(hist_list_Bjets.at(2));
      ...
      for (int j =2; j<10;j++)
	{
	  cout<<hNjets_ratio1->GetBinContent(j)<<endl;

	}
	  hNjets_ratio->Divide(hist_list_Bjets.at(0));

	  hist_list_MET.push_back(hist_list_Bjets.at(0));
	  hist_list_MET.push_back(hist_list_Bjets.at(1));
	  hist_list_MET.push_back(hist_list_Bjets.at(2));
	  hist_list_MET.push_back(hist_list_Bjets.at(3));
	  ...
	  cout<<"\t"<<hist_list_MET.at(0)->GetBinContent(j)<<"\t"<<hist_list_MET.at(1)->GetBinContent(j)<<"\t"<<hist_list_MET.at(2)->GetBinContent(j)<<"\t"<<hist_list_MET.at(3)->GetBinContent(j)<<endl;
	}
