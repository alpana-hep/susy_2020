...
void generate_1Dplot(vector<TH1D*> hist, char const *tag_name="",char const *xlabel="",char const *ylabel="", float energy=-1, int rebin=-1,double ymin=0,double ymax=0,int xmin=-1,int xmax=-1,
...
canvas_n1->SetBottomMargin(0.13);
  ...
  legend = new TLegend(0.15,0.71,0.94,0.945);  
...
void overlay_1file(string pathname, string model, string gluino_m)
{
  ...
  legend_texts ={"No selection","veto lept+isoTracks","p_{T}^{#gamma} > 40","p_{T}^{miss} [GeV] > 100","N_{jets} #geq 2","Sum of p_{T}^{Jets} & p_{T}^{#gamma} > 300 ","d#phi(p_{T}^{miss},lead Jets) > 0.3 ","Preselection","p_{T}^{miss} [GeV] > 200","p_{T}^{miss} [GeV] > 300"};
...
n_files=7;
  ...
  for(int i_file=0; i_file<n_files;i_file++)
      ...
      for(int i_var=0; i_var<n_var;i_var++)
	{
	  ...
	  int xrange=0.0;
	  sprintf(full_path,"%s/%s_%s_%s_%s_overlayBL",pathname.c_str(),varName[i_var].c_str(),model.c_str(),gluino_m.c_str(),Nlsp_m[i_file].c_str());
	  cout<<"varName "<< varName[i_var].c_str() <<"\t"<<Nlsp_m[i_file].c_str()<<"\t"<<i_var<<"\t"<<i_file<<"\t"<<xlabel[i_var].c_str()<<"\t"<<rebin[i_var]<<"\t"<<ymin[i_var]<<"\t"<<ymax[i_var]<<"\t"<<xmin[i_var]<<"\t"<<xmax[i_var]<<"\t"<<legend_title[i_file].c_str()<<endl;
	  if(i_var==2 || i_var==3)
	    generate_1Dplot(hist_list_Njets,full_path,xlabel[i_var].c_str(),"Entries",energy,rebin[i_var],ymin[i_var],ymax[i_var],xmin[i_var],xmax[i_var],false,true,false,true,legend_texts,legend_title[i_file].c_str(), Model);
	  else
	    generate_1Dplot(hist_list_Njets,full_path,xlabel[i_var].c_str(),"Entries",energy,rebin[i_var],ymin[i_var],ymax[i_var],xmin[i_var],xmax[i_var],false,true,false,true,legend_texts,legend_title[i_file].c_str(), Model);
