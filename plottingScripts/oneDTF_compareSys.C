...
//    c->SetBottomMargin(0.12);
  ...
  for(int i =0; i<(int)hist.size();i++) {
    ...
    if(normalize) {
      ...
      hist.at(i)->GetYaxis()->SetTitle("TF = #frac{N_{SR}}{N_{CR}}");
    }
    ...
    // else if(which_TFbins==7)
      hist.at(i)->GetXaxis()->SetRangeUser(1,45);
  ...
  //  if(ymax<=10) ymax=10;
  for(int i =0;i< (int)hist.size(); i++) {
    if(!normalize) hist.at(i)->GetYaxis()->SetRangeUser(0.0,0.1);
    else
...
//     Tl.SetTextSize(0.055);
  ...
  //    hist_ratio.at(0)->SetLineWidth(line_width[0]+1);
    ...
    hist_ratio.at(0)->GetYaxis()->SetTitle("#frac{#DeltaTF}{TF}");//(0#mu,1#gamma)}{(1#mu,1#gamma)}");
    hist_ratio.at(0)->GetXaxis()->SetLabelSize(0.1);
    // if(which_Lept==2)
    //   hist_ratio.at(0)->GetYaxis()->SetRangeUser(0.0,3.5);
    // else
    hist_ratio.at(0)->GetYaxis()->SetRangeUser(-0.1,0.1);
    //    if(which_TFbins==1) //default 8 bins                                                                                                    
    ...
    //   hist_ratio.at(0)->GetXaxis()->SetRangeUser(0,15);
    //  else if(which_TFbins==7)
      hist_ratio.at(0)->GetXaxis()->SetRangeUser(1,46);
   ...
   // else{
      ...
      TLine *l =new TLine(1,0.0,46,0.0);
...
void oneDTF_compareSys(string pathname, int which_Lept, int which_TFBins, bool all, string string_sys)
{
  ...
  legend_texts = {"TF_{nominal}","TF_{nominal+sys}"};//(1l,1#gamma) CR","lost e SR","lost #mu SR","#tau-had SR"};//,"lost #mu SR","lost e SR","(1l,1#gamma) CR"};
  n_files = 18;
  
  cout<<string_png<<"\t"<<TFbins_str<<"\t"<<which_TFBins<<endl;
  if(which_TFBins==1) // btagSFup
    {
      f[0] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_btagSFup_phoID_loose_09Jan24.root");
      f[1] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_phoID_loose_09Jan24.root");
    }
   if(which_TFBins==2) // btagSFdown                                                                                                                               
    {
      f[0] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_btagSFdown_phoID_loose_09Jan24.root");
      f[1] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_phoID_loose_09Jan24.root");
    }
    if(which_TFBins==3) //Jetsys JECup                                                                                                              
    {
      f[0] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_JetSys_JECup_phoID_loose_09Jan24.root");
      f[1] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_phoID_loose_09Jan24.root");
    }
    if(which_TFBins==4) //Jetsys JECdown                                                                                                                              
    {
      f[0] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_JetSys_JECdown_phoID_loose_09Jan24.root");
      f[1] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_phoID_loose_09Jan24.root");
    }
    if(which_TFBins==5) //Jetsys JERup                                                                                                                          
    {
      f[0] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_JetSys_JERup_phoID_loose_09Jan24.root");
      f[1] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_phoID_loose_09Jan24.root");
    }
    if(which_TFBins==6) //Jetsys JERdown
      {
      f[0] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_JetSys_JERdown_phoID_loose_09Jan24.root");
      f[1] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_phoID_loose_09Jan24.root");
    }

    if(which_TFBins==7) //Pileup Up                                                                                               
      {
	f[0] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_pileup_sys_up_phoID_loose_09Jan24.root");
	f[1] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_phoID_loose_09Jan24.root");
      }
    if(which_TFBins==8) //Pileup down                                                                                                   
      {
	f[0] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_pileup_sys_down_phoID_loose_09Jan24.root");
	f[1] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_phoID_loose_09Jan24.root");
      }

    if(which_TFBins==9) //CrossSect Up                                                                                                                   
      {
        f[0] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_CrossSecUp_phoID_loose_09Jan24.root");
        f[1] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_phoID_loose_09Jan24.root");
      }
    if(which_TFBins==10) //CrossSec down
      {
        f[0] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_CrossSecDown_phoID_loose_09Jan24.root");
        f[1] = new TFile("Electron_FR_TFbins_v3_phopt_qmulti_phoID_loose_09Jan24.root");
      }
 ...
 //  if(which_Lept==3){
    ...
    filetag={"W+TTBar_2016preVFP","W+TTBar_2016postVFP","W+TTBar_2017","W+TTBar_2018","W+TTBar_FullRun2","TTGJets+TTJets_2016preVFP","TTGJets+TTJets_2016postVFP","TTGJets+TTJets_2017","TTGJets+TTJets_2018","TTGJets+TTJets_Run2","WGJets+WJets_2016preVFP","WGJets+WJets_2016postVFP","WGJets+WJets_2017","WGJets+WJets_2018","WGJets+WJets_Run2","WGJets+WJets_2016","TTGJets+TTJets_2016","W+TTBar_2016"};
 filetag=  {"TTGJets_2018","TTGJets_2017","TTGJets_2016postVFP","Run2_TTGJets","WGJets_2018","WGJets_2017","WGJets_2016postVFP","Run2_WGJets","TTJets_2018","TTJets_2017","TTJets_2016postVFP","Run2_TTJets","WJets_2018","WJets_2017","WJets_2016postVFP","Run2_WJets","WGJets+WJets_2018","WGJets+WJets_2017","WGJets+WJets_2016postVFP","Run2_WGJets+WJets","TTGJets+TTJets_2018","TTGJets+TTJets_2017","TTGJets+TTJets_2016postVFP","Run2_TTGJets+TTJets","W+TTBar_2018","W+TTBar_2017","W+TTBar_2016postVFP","W+TTBar_FullRun2","TTGJets_2016preVFP","WGJets_2016preVFP","TTJets_2016preVFP","WJets_2016preVFP","WGJets+WJets_2016preVFP","TTGJets+TTJets_2016preVFP","W+TTBar_2016preVFP","TTGJets_2016","WGJets_2016","TTJets_2016","WJets_2016","WGJets+WJets_2016","TTGJets+TTJets_2016","W+TTBar_2016"};
 energyy={59.74,41.529,16.5,137.19,59.74,41.529,16.5,137.19,59.74,41.529,16.5,137.19,59.74,41.529,16.5,137.19,59.74,41.529,16.5,137.19,59.74,41.529,16.5,137.19,59.74,41.529,16.5,137.19,19.5,19.5,19.5,19.5,19.5,19.5,19.5,36.0,36.0,36.0,36.0,36.0,36.0,36.0};
  ...
  for(int i_file=0; i_file<filetag.size();i_file++)
    {
      ...
      sprintf(hist_name,"h_TFbins_LL_%s",filetag[i_file].c_str());
      cout<<"reading original TF"<<"\t"<<hist_name<<"\t"<<f[0]->GetName()<<"\t"<<endl;
      TH1F* h_TF_orig = (TH1F*)f[1]->Get(hist_name);
	...
	h_nominalwrtTF1->SetBinError(j,h_TF_orig->GetBinError(j)/h_TF_orig->GetBinContent(j));
      ...
      generate_1Dplot(hist_list_Njets,hist_list_MET,full_path,energy,40,0,leg_head,false,false,false,true,string_sys.c_str(),legend_texts,which_TFBins, which_Lept);
