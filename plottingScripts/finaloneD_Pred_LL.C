...
// /int line_color[n_pl+1] = {kBlack, kRed, kGreen+2, kBlue, kRed};                                                                               
int line_color[9] = {kGreen+1,kRed,kGreen+2,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2};//{9,kCyan+2,45,kMagenta,kGray+1,kRed,kBlue+2,kMagenta,kCyan};
int line_color1[9]= {kGreen+1,kRed,kGreen+2,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2};
int line_color2[9] = {kGreen+1,kRed,kBlue,kGreen+2,kGray,kViolet+2,kGreen-2,kYellow+1,kGray+2};//,kMagenta};
//int line_color[9] = {kMagenta+2, kGray+2, kRed, kGreen+2, kMagenta, kRed - 3, kBlue + 2 , kCyan + 1 , kGreen + 3 };
vector<int> col={kGreen+1,kRed,kViolet,kGray,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
...
void decorate(TH1F* hist,int i, int j){
  ...
  //   hist->SetFillColor(col[i]);
   ...
   vector<int> Style;
   if(j==1){
     col={kGreen+1,kViolet,kGreen+2,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
     Style= {1001,3008,3019,3244};

   }
   else if(j==2){
     col={kGreen+1,kBlue,kGreen+2,kBlue,kGreen+2,kGray+1,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
     Style={1001,1001,3008,1001,3244};
   }
   else if(j==3){
     col={kGreen+1,kViolet,kBlue,kGreen+2,kGreen+2,kBlue,kViolet,kGray,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
...
void finaloneD_Pred_LL(string pathname, int which_Lept, int which_TFBins, bool all, string string_sys)
{
  ...
  // }
  // if(which_Lept==3){
  sprintf(string_png,"Lepton_LL_%s_%s",TFbins_str, string_sys.c_str());
  sprintf(hname,"%s",string_png);
  baseline1 = {"Elec_CR","Mu_CR","Elec_SR","Mu_SR","TauHad_SR"};//,"Mu_SR","Elec_SR"};
  legend_texts = {"Predictions","Systematic unc.","Statistical unc.","Systematic unc."};//(1l,1#gamma) CR","lost e SR","lost #mu SR","#tau-had SR"};//,"lost #mu SR","lost e SR","(1l,1#gamma) CR"};
  n_files = 18;
  ...
  //   {
      f[0] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/out_Data_FullRun2_Allruns_MET_phoID_loose_pt40_MET200_Lepton_HEMveto_PU_L1prefire.root");
      ...
      f[4] = new TFile("Lepton_LL_TFbins_v7_HTbjetsPhopt_CrossSection_Sys_up.root");
      f[5] = new TFile("Lepton_LL_TFbins_v7_HTbjetsPhopt_Pileup_Sys_up.root");     
      n_files=6;
   //  }
