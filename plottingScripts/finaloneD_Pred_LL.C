...
// /int line_color[n_pl+1] = {kBlack, kRed, kOrange+2, kBlue, kRed};                                                                               
int line_color[9] = {kOrange+1,kBlue+1,kGray+2,kViolet+2,kOrange-2,kYellow+1,kGray+2,kMagenta,kBlue+2};//{9,kCyan+2,45,kMagenta,kGray+1,kRed,kBlue+2,kMagenta,kCyan};
int line_color1[9]= {kOrange+1,kRed,kGray+2,kViolet+2,kOrange-2,kYellow+1,kGray+2,kMagenta,kBlue+2};
int line_color2[9] = {kOrange+1,kRed,kBlue,kOrange+2,kGray,kViolet+2,kOrange-2,kYellow+1,kGray+2};//,kMagenta};
//int line_color[9] = {kMagenta+2, kGray+2, kRed, kOrange+2, kMagenta, kRed - 3, kBlue + 2 , kCyan + 1 , kOrange + 3 };
vector<int> col={kOrange+1,kRed,kViolet,kGray,kViolet+2,kOrange-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
vector<int> Style={1001,1001,3008,1001};
...
void decorate(TH1F* hist,int i, int j){
  ...
  //   hist->SetFillColor(col[i]);
   ...
   vector<int> Style;
   if(j==1){
     col={kOrange+1,kViolet,kOrange+2,kViolet+2,kOrange-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
     Style= {1001,3008,3019,3244};

   }
   else if(j==2){
     col={kOrange+1,kBlue,kOrange+2,kBlue,kOrange+2,kGray+1,kViolet+2,kOrange-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
     Style={1001,1001,3008,1001,3244};
   }
   else if(j==3){
     col={kOrange+1,kViolet,kBlue,kOrange+2,kOrange+2,kBlue,kViolet,kGray,kViolet+2,kOrange-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
...
void generate_1Dplot(vector<TH1F*> hist, vector<TH1F*> hist_ratio, char const *tag_name="", float energy=-1, int xrange=-1,int xmin=-1,char const *leg_head="",
   ...
   TCanvas *canvas_n1 = new TCanvas(tag_name, tag_name,1200,1000);//600,600,1200,1200);
   canvas_n1->Range(-60.25,-0.625,562.25,0.625);
   canvas_n1->SetFillColor(0);
   canvas_n1->SetBorderMode(0);
   canvas_n1->SetBorderSize(2);
   canvas_n1->SetRightMargin(0.02);                                                                                                                               
   canvas_n1->SetLeftMargin(0.14);       
   //latest
...
//    c->SetBottomMargin(0.12);
  ...
  for(int i =0; i<(int)hist.size();i++) {
    ...
    if(normalize) {
      ...
      hist.at(i)->GetYaxis()->SetTitle("Events");
    }
    ...
    hist.at(i)->GetXaxis()->SetTitle("Bin No.");
     hist.at(i)->GetXaxis()->SetTitleOffset(1);
  ...
  for(int i =0;i< (int)hist.size(); i++) {
    ...
    if(i==0) {
      hist.at(0)->SetLineColor(kOrange+1);
      hist.at(0)->SetMarkerColor(kBlack);
      hist.at(0)->SetMarkerSize(1.0);
      hist.at(0)->SetMarkerStyle(8);
      hist.at(0)->Draw("BAR HIST P");
    }
       
	hist.at(1)->SetLineColor(kGray+6);
	hist.at(1)->SetMarkerColor(kGray+6);
	hist.at(1)->SetMarkerSize(0.01);
	
	hist.at(1)->SetFillColor(kBlack);
	hist.at(1)->SetFillStyle(3013);
	hist.at(1)->Draw("e2 same");

  
  }
  legName.push_back(hist.at(0)->GetName());
  hist.at(0)->SetLineColor(kBlack);
  hist.at(0)->Draw("ePsames");
  leg_entry[2] = legend->AddEntry(hist.at(0),legend_texts[2].c_str(),"ep");
  leg_entry[2]->SetTextColor(kBlack);
...
//     hs_var->GetYaxis()->SetTitleOffset(1.0);
  ...
  textOnTop->DrawLatexNDC(0.68,0.925,en_lat);
// TLatex Tl;                                                                                                                                                       
...
void finaloneD_Pred_LL(string pathname, int which_Lept, int which_TFBins, bool all, string string_sys)
{
  ...
  // if(which_TFBins==1)
  sprintf(TFbins_str,"TFbins_v7_HTbjetsPhopt");
  // else if (which_TFBins==2)
  ...
  // }
  // if(which_Lept==3){
  sprintf(string_png,"FakePhoton_%s_%s",TFbins_str, string_sys.c_str());
  sprintf(hname,"%s",string_png);
  baseline1 = {"h_Sbins_LL_newSbins_Validation_v7_Elec_CR","Mu_CR"};//,"Elec_SR","Mu_SR","TauHad_SR"};//,"Mu_SR","Elec_SR"};
  legend_texts = {"Predictions","Systematic unc.","Statistical unc."};//(1l,1#gamma) CR","lost e SR","lost #mu SR","#tau-had SR"};//,"lost #mu SR","lost e SR","(1l,1#gamma) CR"};
  n_files = 18;

  if(which_Lept==3){
      baseline1 = {"h_Sbins_LL_newSbins_Validation_v7_Elec_CR","newSbins_Validation_v7_Mu_CR","newSbins_v7_TauHad_SR","newSbins_v7_Mu_SR","newSbins_v7_Elec_SR"};
  ...
  if(which_TFBins==1) // btagSFup
    {
      f[0] = new TFile("out_Data_FullRun2_Allruns_MET_phoID_loose_pt40_MET200.root");
      f[1] = new TFile("Lepton_LL_TFbins_v7_HTbjetsPhopt_btag_SF_up.root");
      f[2] = new TFile("Lepton_LL_TFbins_v7_HTbjetsPhopt_JEC_Sys_up.root");
      f[3] = new TFile("Lepton_LL_TFbins_v7_HTbjetsPhopt_JER_Sys_up.root");
      f[4] = new TFile("Lepton_LL_TFbins_v7_HTbjetsPhopt_Pileup_Sys_up.root");
      n_files=5;
    }
   //         f[1] = new TFile("Lepton_LL_TFbins_v7_HTbjetsPhopt_btag_SF_up.root");
   //    n_files=2;
   ...
   //  {
   //     f[0] = new TFile("btagSFSys/Down/Pred/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloosebtagSFup_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //    f[1] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/WithBtagging/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloose_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");

   //  }
   //  if(which_TFBins==3) //Jetsys JECup                                                                                                              
   //  {
   //     f[0] = new TFile("JECSys/Up/Pred/FullRun2_WGJets_TTGJets_Allcombined_PhoIdlooseJetSys_JECup_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //    f[1] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/WithBtagging/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloose_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");

   //  }
   //  if(which_TFBins==4) //Jetsys JECdown                                                                                                                              
   //  {
   //    f[0] = new TFile("JECSys/Down/Pred/FullRun2_WGJets_TTGJets_Allcombined_PhoIdlooseJetSys_JECdown_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //    f[1] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/WithBtagging/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloose_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");

   //  }
   //  if(which_TFBins==5) //Jetsys JERup                                                                                                                          
   //  {
   //    f[0] = new TFile("JERSys/Up/Pred/FullRun2_WGJets_TTGJets_Allcombined_PhoIdlooseJetSys_JERup_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //    f[1] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/WithBtagging/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloose_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");

   //  }
   //  if(which_TFBins==6) //Jetsys JERdown
   //    {
   // 	f[0] = new TFile("JERSys/Down/Pred/FullRun2_WGJets_TTGJets_Allcombined_PhoIdlooseJetSys_JERdown_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   // 	f[1] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/WithBtagging/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloose_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //  }

   //  if(which_TFBins==7) //Pileup Up                                                                                               
   //    {
   // 	     f[0] = new TFile("puSys/Up/Pred/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloosepuSysUp_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   // 	     f[1] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/WithBtagging/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloose_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //    }
   //  if(which_TFBins==8) //Pileup down                                                                                                   
   //    {
   // 	 f[0] = new TFile("puSys/Down/Pred/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloosepuSysDown_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   // 	 f[1] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/WithBtagging/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloose_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //    }

   //  if(which_TFBins==9) //CrossSect Up                                                                                                                   
   //    {
   // 	 f[0] = new TFile("CrossSection_Sys/Up/Pred/FullRun2_WGJets_TTGJets_Allcombined_PhoIdlooseCrossSecUp_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //       f[1] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/WithBtagging/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloose_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");

   //    }
   //  if(which_TFBins==10) //CrossSec down
   //    {
   // 	f[0] = new TFile("CrossSection_Sys/Down/Pred/FullRun2_WGJets_TTGJets_Allcombined_PhoIdlooseCrossSecDown_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //       f[1] = new TFile("/media/alpana/B6C0-6C7D/Alpana/Work/Susy_GMSB_lowPtPho_Analysis/January2024_LL_Studies_WminorCorrect/Oct24_newValidationBins/WithBtagging/FullRun2_WGJets_TTGJets_Allcombined_PhoIdloose_phopt40_MET200_LostLepton_HEMveto_PU_L1prefire.root");
   //    }
  ...
  for(int i_file=4; i_file<5;i_file++)
    {
      ...
      sprintf(hist_name,"h_TFbins_LL_W+TTBar_FullRun2");//h_TFbins_LL_%s",filetag[i_file].c_str());
      cout<<"reading original TF"<<"\t"<<hist_name<<endl;
      TH1F* h_TF_orig = (TH1F*)hist_list_Bjets.at(0)->Clone();
      //      h_TF_orig->Add(hist_list_Bjets.at(1));
      sprintf(hist_name,"h_TFbins_LL_W+TTBar_FullRun2");
      TH1F* h_TF_new[10];
      for(int iff=1;iff<5;iff++){
      h_TF_new[iff] = (TH1F*)f[iff]->Get(hist_name);
      }
      cout<<"reading h new read"<<endl;
      double TF_orig, ratio, delta, statsErr, sysError;
      double TF_new[10]={};
      ...
      TH1F* h_nominalwrtTF1 = (TH1F*)h_TF_orig->Clone();
         for(int j=2;j<102; j++){
        TF_orig = h_TF_orig->GetBinContent(j); // original Predicted values                                                                                           
        statsErr = h_TF_orig->GetBinError(j); // statistical error                                                                                                    
        cout<<"reading h new read-1"<<"\t"<<h_TF_orig->GetBinContent(j)<<"\t"<<h_TF_new[1]->GetBinContent(j)<<endl;
         for(int iff=1;iff<5;iff++){
        TF_new[iff] = h_TF_new[iff]->GetBinContent(j);
	cout<<j<<"\t"<<ratio<<"\t"<<TF_orig<<"\t"<<TF_new<<"\t"<<sysError<<"\t"<<endl;
         }
         sysError = sqrt(TF_new[1]*TF_new[1]+TF_new[2]*TF_new[2]+TF_new[3]*TF_new[3]+TF_new[4]*TF_new[4]+TF_new[5]*TF_new[5]+0.03*0.03+0.12*0.12);
        cout<<j<<"\t"<<ratio<<"\t"<<TF_orig<<"\t"<<TF_new<<"\t"<<sysError<<"\t"<<endl;
	h_nominalwrtTF->SetBinContent(j,TF_orig);
        h_nominalwrtTF->SetBinError(j,sysError*h_TF_orig->GetBinContent(j));
	...
	h_nominalwrtTF1->SetBinError(j,h_TF_orig->GetBinError(j)/h_TF_orig->GetBinContent(j));
      ...
      // 	TF_orig = h_TF_orig->GetBinContent(j); // original Predicted values 
      // 	statsErr = h_TF_orig->GetBinError(j); // statistical error
      // 	cout<<"reading h new read-1"<<"\t"<<h_TF_new[1]->GetBinContent(j)<<endl;
      // 	TF_new[0] = h_TF_new[1]->GetBinContent(j);
      // 	 cout<<"reading h new read-2"<<endl;
      // 	TF_new[1] = h_TF_new[1]->GetBinContent(j);
      // 	sysError = sqrt(TF_new[0]*TF_new[0]+TF_new[1]*TF_new[1]);
      // 	// delta = TF_orig-TF_new;
      // 	// ratio = delta/TF_orig;
      // 	cout<<j<<"\t"<<ratio<<"\t"<<TF_orig<<"\t"<<TF_new<<"\t"<<sysError<<"\t"<<endl;
      // 	h_nominalwrtTF->SetBinContent(j,TF_orig);
      // 	h_nominalwrtTF->SetBinError(j,sysError);
      // 	h_nominalwrtTF1->SetBinContent(j,0);
      // 	h_nominalwrtTF1->SetBinError(j,h_TF_orig->GetBinError(j)/h_TF_orig->GetBinContent(j));
      // }
      hist_list_Njets.push_back(h_TF_orig);
      //      hist_list_Njets.push_back(h_TF_new);
      hist_list_Njets.push_back(h_nominalwrtTF);
	...
	//    sprintf(canvas_name,"temp.png");//,tag_name, filetag[i_file].c_str());
      ...
      generate_1Dplot(hist_list_Njets,hist_list_MET,full_path,energy,40,0,leg_head,false,true,false,true,string_sys.c_str(),legend_texts,which_TFBins, which_Lept);
