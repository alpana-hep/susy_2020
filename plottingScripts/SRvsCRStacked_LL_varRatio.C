...
// /int line_color[n_pl+1] = {kBlack, kRed, kGreen+2, kBlue, kRed};                                                                               
int line_color[10] = {kGray+1,kPink-9,kViolet,kGreen+2,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2};//{9,kCyan+2,45,kMagenta,kGray+1,kPink-9,,kRed,kBlue+2,kMagenta,kCyan};
int line_color1[10]= {kGray+1,kPink-9,kBlue,kGreen+2,kViolet+2,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2};
int line_color2[10] = {kGray+1,kPink-9,kViolet,kBlue,kGreen+2,kGray,kViolet+2,kGreen-2,kYellow+1,kGray+2};//,kMagenta};
...
//    c->SetBottomMargin(0.12);
  ...
  legend = new TLegend(0.2,0.75,0.75,0.88);  
...
//     hs_var->GetYaxis()->SetTitleOffset(1.0);
  ...
  hs_var->GetXaxis()->SetLabelSize(0.04);
  hs_var->GetYaxis()->SetLabelSize(0.04);
  hs_var->GetYaxis()->SetTitleSize(00.04);
  hs_var->GetYaxis()->SetTitleOffset(1.0);
  //new ones
...
void SRvsCRStacked_LL_varRatio(string pathname, int which_Lept, int which_TFBins, bool all)
{
...
char* hname2 = new char[200];
  ...
  if(which_TFBins==1)
    ...
    sprintf(TFbins_str,"TFbins_v8_nBjets_HT_PhoPt");
  else if(which_TFBins==3)
    sprintf(TFbins_str,"TFbins_v8_nBjets_HT_PhoPt");
  ...
  if(which_Lept==3){
    ...
    legend_texts = {"(1l,1#gamma) CR","(0l, #gamma) SR","lost #mu SR","#tau-had SR"};//,"lost #mu SR","lost e SR","(1l,1#gamma) CR"};
    n_files = 18;
  }
...
P","TTGJets+TTJets_2017","TTGJets+TTJets_2018","TTGJets+TTJets_FullRun2","WGJets+WJets_2016preVFP","WGJets+WJets_2016postVFP","WGJets+WJets_2017","WGJets+WJets_2018","WGJets+WJets_FullRun2","WGJets+WJets_2016","TTGJets+TTJets_2016","W+TTBar_2016"};
  ...
  for(int i_file=0; i_file<n_files;i_file++)
      ...
      for(int i_cut=0; i_cut<baseline1.size();i_cut++)
	{
      ...
      TH1F* hMEt = (TH1F*)hNjets_ratio->Clone();
      hNjets_ratio->Divide(hNjets_ratio2);
      // yaha pe change karna h
      hist_list_MET.push_back(hNjets_ratio2);
      hist_list_MET.push_back(hMEt);
	...
	TH1F* hMEt = (TH1F*)hNjets_ratio->Clone();
	hist_list_MET.push_back(hMEt);
	hist_list_MET.push_back(hist_list_Njets.at(0));
	hNjets_ratio->Divide(hist_list_Njets.at(0));
