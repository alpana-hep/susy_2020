...
void AnalyzeLightBSM::EventLoop(const char *data,const char *inputFileList, const char *sample , const char *outFileName,  const char* phoID) {
 ...
 //  // reader1->BookMVA( "BDT_100trees_2maxdepth method", "TMVAClassification_BDT_100trees_2maxdepth.weights.xml");
  ...
  // sprintf(basedir,"/home/kalpana/t3store3/public/SUSY_GMSB_Aug_2020/CMSSW_11_1_0_pre3/src/Susy_Analysis_2020");//root://cmseos.fnal.gov.in//store/user/kalpana/Susy_phoMet/SkimmedNtuples");                                                                                                                                             
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)
	...
	TAPElectronTracks_v1 = (*TAPElectronTracks);
      ...
      pre_evtwt= 0.0;
      pre_dPhi_Met_Jet1=0.0;
      pre_dPhi_Met_Jet2=0.0;
      pre_dPhi_Pho_Jet1=0.0;
      pre_dPhi_Pho_Jet2=0.0;

      pre_MET=0.0;
      pre_METPhi=0.0;
      pre_LeadJets1Pt=0.0;
      pre_LeadJets1Eta=0.0;
      pre_LeadJets1Phi=0.0;
      pre_LeadJets1E=0.0;
      pre_LeadJets2Pt=0.0;
      pre_LeadJets2Eta=0.0;
      pre_LeadJets2Phi=0.0;
      pre_LeadJets2E=0.0;
      pre_SusyLSPMass = 0.0;
...
// // ********* This is to account into all visible energy: Adding photon matching with Jet ******************//
    ...
    pre_Weight = Weight;
    pre_Nentries = nentries;
    pre_NhadJets = nHadJets;
    pre_NbJets= BTags;
    pre_Nphotons = nPhotons;
    pre_PhoPt= bestPhoton.Pt();
    pre_PhoEta= bestPhoton.Eta();
    pre_PhoPhi= bestPhoton.Phi();
    pre_PhoE= bestPhoton.E();
    ...
    pre_evtwt= wt;
    pre_dPhi_Met_Jet1=dPhi_METjet1;
    pre_dPhi_Met_Jet2=dPhi_METjet2;
    pre_dPhi_Pho_Jet1=abs(bestPhoton.DeltaPhi(hadJets[0]));
    pre_dPhi_Pho_Jet2=abs(bestPhoton.DeltaPhi(hadJets[1]));

    pre_MET=MET;
    pre_METPhi=METPhi;
    pre_LeadJets1Pt=hadJets[0].Pt();
    pre_LeadJets1Eta=hadJets[0].Eta();
    pre_LeadJets1Phi=hadJets[0].Phi();
    pre_LeadJets1E=hadJets[0].E();
    pre_LeadJets2Pt=hadJets[1].Pt();
    pre_LeadJets2Eta=hadJets[1].Eta();
    pre_LeadJets2Phi=hadJets[1].Phi();
    pre_LeadJets2E=hadJets[1].E();
    pre_SusyMotherMass = SusyMotherMass;
    pre_SusyLSPMass = SusyLSPMass;
    pre_BestPhoton=goodPho_n;
    pre_hadJets=hadJets;
    skim_tree->Fill();  
    if(Debug)
