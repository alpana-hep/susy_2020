...
int line_width[12] = {3,3,2,2,3,2,2,2,2,2,2,2};
int line_style[12] = {2,2,1,1,2,1,1,1,1,1,1,1};
...
canvas_n1->SetBottomMargin(0.13);
  ...
  legend = new TLegend(0.15,0.78,0.9,0.945);  
 ...
 // axis51->SetRangeUser(0,2000);
  ...
  mg->GetXaxis()->SetRangeUser(xmin,2000);
...
void overlay_multifile(string pathname, string model, int which_plot)
{
 ...
 for(int i_cut=0; i_cut<n_cut;i_cut++){
   ...
   for(int i_var=0; i_var<n_var;i_var++)
	  ...
	  generate_1Dplot(hist_list_Njets,full_path,xlabel[i_var].c_str(),ylabel[i_var].c_str(),energy,rebin[i_var],ymin[i_var],ymax[i_var],xmin[i_var],xmax[i_var],legend_texts[i_cut].c_str(),false,true,false,true,legend_texts_v1,legend_title[0].c_str(), Model);
