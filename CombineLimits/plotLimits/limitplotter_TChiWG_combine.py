...
#print  filename
infile=TFile(inputfoldername,"READ")
#    infile.ls()
#print
tr=infile.Get("limit")
#    tr.Show(0)
for ie,event in enumerate(tr):
    ...
    print (ie,round(event.quantileExpected,2),round(10000*event.mh,0),event.limit)
    if round(10000*event.mh,0)>1475:
        continue
    ...
    if round(event.quantileExpected,2) == 0.03 : m2sigmaexparray.append((round(10000*event.mh,0),event.limit))
    if round(event.quantileExpected,2) == 0.16 : m1sigmaexparray.append((round(10000*event.mh,0),event.limit))
    if round(event.quantileExpected,2) == 0.5  : meanexparray.append((round(10000*event.mh,0),event.limit))
    if round(event.quantileExpected,2) == 0.84 : p1sigmaexparray.append((round(10000*event.mh,0),event.limit))
    if round(event.quantileExpected,2) == 0.98 : p2sigmaexparray.append((round(10000*event.mh,0),event.limit))
    if round(event.quantileExpected,2) == -1.0 : obsarray.append((round(10000*event.mh,0),event.limit))
    if round(event.quantileExpected,2) == -1.0 : masserrorarray.append(0.0)


#print
#print
print(len(meanexparray))
print ("got limits into array")
for ii in meanexparray:
   print ( ii)
...
legend2.AddEntry(theorycurve,"#tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{0} + #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{#mp}","FL")
legend2.AddEntry(theorycurve2,"#tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{0}","FL")  
#legend2.AddEntry(theorycurve2,"#tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{0} + #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{#pm} with unc.","FL")  
#legend2.AddEntry(theorycurve,"#sigma_{#tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{0} + #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{#pm}} with unc.","FL")
#legend2.AddEntry(theorycurve2,"#sigma_{#tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{0}} with unc.","FL")

#legend1 = TLegend(0.22,0.18,0.75,0.2)
#legend1_ = TLegend(0.3,0.24,0.5,0.26)
legend1 = TLegend(0.36,0.18,0.5,0.2)
...
#legend1_ = TLegend(0.11,0.24,0.5,0.26)
...
#legend1_.SetBorderSize(0)
#legend1_.SetTextSize(0.032); 
#legend1_.SetHeader("pp #rightarrow #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{0} / #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{#mp}","C")
legend1_.SetHeader("pp #rightarrow #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{0}","C")
#legend1.SetHeader("#tilde{#chi}_{1}^{#pm} #rightarrow W^{#pm} #tilde{G} , #tilde{#chi}_{1}^{0} #rightarrow #gamma #tilde{G}","C")
legend1.SetHeader("#it{B}(#tilde{#chi}_{1}^{#pm}#rightarrow W^{#pm} #tilde{G}) = #it{B}(#tilde{#chi}_{1}^{0}#rightarrow #gamma #tilde{G}) = 100%","C")
#legend = TLegend(0.52,0.56,0.83,0.76)
#legend = TLegend(0.52,0.52,0.8,0.7)
legend = TLegend(0.52,0.56,0.83,0.76)
legend.SetFillColor(0); legend.SetLineColor(0); legend.SetFillColor(0); legend.SetFillStyle(0)
legend.SetTextSize(0.03); legend.SetTextFont(42); legend.SetTextAlign(12)
...
observedlimit.SetMarkerSize(12)
...
#cmslabel.DrawLatexNDC(0.16,0.942,"CMS #it{#bf{}}");
...
can.SaveAs("./"+limitname+".pdf")
can.SaveAs("./"+limitname+".png")
