...
void makeDatacard_SBins(double mGl,double mNLSP,TString sigFile,string histname1,string histname){//[1000],char histname[1000]){
  char name[1000];
  ...
  f[1] = new TFile("FullRun2_WGJets_PhoIdloose_phopt40_BL_BDTwithT5bbbbZg_7variables.root");
  f[2] = new TFile("FullRun2_WJetsToLNu_HT_PhoIdloose_phopt40_BL_BDTwithT5bbbbZg_7variables.root");
  f[3] = new TFile("FullRun2_TTJets_PhoIdloose_phopt40_BL_BDTwithT5bbbbZg_7variables.root");
  f[4] = new TFile("FullRun2_TTGJets_inc_PhoIdloose_phopt40_BL_BDTwithT5bbbbZg_7variables.root");
  f[5] = new TFile("FullRun2_ZNuNu_PhoIdloose_phopt40_BL_BDTwithT5bbbbZg_7variables.root");
  f[6] = new TFile("FullRun2_GJets_QCD_PhoIdloose_phopt40_BL_BDTwithT5bbbbZg_7variables.root");
  ...
  for(int i=0;i<nFiles;i++){
    hist1[i] = (TH1D*)f[i]->FindObjectAny(histname.c_str());
  ...
  for(int i=1;i<=imax;i++){
    ...
    for(int j=0;j<jmax+nSig;j++){
      if(hist1[j]->GetBinContent(i) == 0) {   cout<<"file name : "<<getfname(f[0]->GetName())<<endl;
	counter++;
      }      
    }
    if (counter ==jmax+nSig) { flag_write=false; cout<<"total N files has 0 bin content "<<counter<<endl;}
