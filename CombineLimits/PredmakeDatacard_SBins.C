...
void PredmakeDatacard_SBins(double mGl,double mNLSP,TString sigFile,string histname1,string histname, string modelname){//[1000],char histname[1000]){
  char name[1000];
  ...
  sprintf(fnames,"PredData_FullRun2_Allruns_MET_LostLepton.root");//ullRun2_WGJets_PhoIdloose_phopt40_BL_BDTwith13variables_%s.root",modelname.c_str());
  f[1] = new TFile(fnames);
  sprintf(fnames,"PredData_FullRun2_Allruns_MET_FakePhoton.root");//0_BL_BDTwith13variables_%s.root",modelname.c_str());
  f[2] = new TFile(fnames);
  sprintf(fnames,"PredData_FullRun2_Allruns_MET_Zinvi.root");//FullRun2_TTJets_PhoIdloose_phopt40_BL_BDTwith13variables_%s.root",modelname.c_str());
  f[3] = new TFile(fnames);
  sprintf(fnames,"FullRun2_GJets_QCD_PhoIdloose_phopt40_BL.root");//FullRun2_GJets_QCD_PhoIdloose_phopt40_BL_BDTwith13variables_%s.root",modelname.c_str());
  f[4] = new TFile(fnames);
  ...
  for(int i=0;i<nFiles;i++){
    if(i<=2 && i!=0){
      if(i==2)
      hist1[i] = (TH1D*)f[i]->FindObjectAny("h_Sbins_LL_newSbins_Validation_v7_Elec_CR");
    if(i==1){
      hist1[i] = (TH1D*)f[i]->FindObjectAny("h_Sbins_LL_v4_newSbins_Validation_v7_Elec_CR");
      if(i==1){hist3[i] =(TH1D*)f[i]->FindObjectAny("h_Sbins_LL_v4_newSbins_Validation_v7_Mu_CR"); hist1[i]->Add(hist3[i]); }}}
    else if(i==3)  hist1[i] = (TH1D*)f[i]->FindObjectAny("h_Sbins_LL_newSbins_v7_Pred_SR");
    else {cout<<i<<"\t"<<f[i]->GetName()<<endl;  hist1[i] = (TH1D*)f[i]->FindObjectAny(histname.c_str()); }//h_Sbins_LL_newSbins_v7_Pred_SR h_Sbins_LL_newSbins_Validation_v7_Elec_CR
  ...
  cout<<"file name1 : "<<getfname(f[0]->GetName())<<endl;
  for(int i=1;i<=imax;i++){
    bool flag_write=true;
    int counter=0;
    for(int j=0;j<jmax+nSig;j++){
      //      cout<<"file name---- : "<<getfname(f[j]->GetName())<<"\t"<<i<<"\t"<<hist1[j]->GetName()<<endl;
    ...
    //    cout<<"name2"<<name2<<"\t"<<histname<<"\t"<<endl;
    ...
    outf<<endl<<
      "rate ";
    for(int j=0;j<jmax+nSig;j++){
      //if(hist1[j]->GetBinContent(i) >= 0){ cout<<"just before bin content "<<hist1[j]->GetName()<<"\t"<<j<<jmax+nSig<<endl;}
    ...
    for(int j1=0;j1<jmax+nSig;j1++){
      ...
      //cout<<getfname(f[j1]->GetName())<<"bin"<<i<<" lnN "<<endl;
      for(int j2=0;j2<jmax+nSig;j2++){
	if(j1==j2){
	  //  outf<<"1.20 ";
	  //  if((hist1[j2]->GetBinContent(i))>0.00000001) cout<<"final step "<<endl;
