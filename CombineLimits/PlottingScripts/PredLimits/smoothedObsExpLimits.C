...
TFile *fout =new TFile("Excl_v1_"+fName,"recreate");
 ...
 */
 if(modelName.Contains("T5bbbb") || modelName.Contains("T5qqqq")){ xMin = 1800; xMax = 2650; nDivAxis = 510; yMin = 0.; yMax = 4000.; zMin = 1.1e-1, zMax = 40e-1;}
if(modelName.Contains("T5tttt")                              )  { xMin = 1800; xMax = 2450; nDivAxis = 510;}
if(modelName.Contains("T6tt")                                )  { xMin =  1000; xMax = 1600; nDivAxis = 510; yMin = 0.; yMax = 2000.;}
if(modelName.Contains("TChi")                                )  { xMin =  200; xMax = 1300; nDivAxis = 510; yMin = 0.; yMax = 800.;}

TPaveText *decayMode = new TPaveText(0.15,0.75, 0.8,0.9,"NDC");
...
TCanvas *c3=new TCanvas("c3","c3",1300,1100);
...
*/
 gr1d_Median->SetLineColor(kBlack);
...
//gr2dXsec->GetXaxis()->SetNdivisions(505);
 ...
 gr2dXsec->GetXaxis()->SetNdivisions(505);
h2_XsecUL->GetXaxis()->SetNdivisions(505);
...
//h2_XsecUL->GetZaxis()->SetTitleOffset(0.);

gr2dXsec->GetZaxis()->SetTitle("95% CL upper limit on cross section (fb)");
h2_XsecUL->GetZaxis()->SetTitle("95% CL upper limit on cross section (fb)");
...
// gr2dXsec->GetZaxis()->SetTitleOffset(0.8);                                                                                                                         
 gr2dXsec->GetZaxis()->SetTitleOffset(1.);
 //gr2dXsec->SetStats(0);
...
gPad->RedrawAxis();
//-----------------------------------------------
 decayMode->SetTextSize(0.033); 
if(modelName.Contains("T5qqqqHg")){
  decayMode->AddText("#bf{pp #rightarrow #tilde{g} #tilde{g}, #tilde{g} #rightarrow q #bar{q} #tilde{#chi}_{ 1}^{ 0}, B(#tilde{#chi}_{ 1}^{ 0} #rightarrow #gamma #t\
ilde{G}) = B(#tilde{#chi}_{ 1}^{ 0} #rightarrow H #tilde{G})=50%}");// #tilde{#chi}_{ 1}^{ 0} #rightarrow #gamma/H #tilde{G}}");//  NLO+NLL exclusion}");
  modelName="T5qqqqHg";}
 else if(modelName.Contains("T5ttttZg")){
   decayMode->AddText("#bf{pp #rightarrow #tilde{g} #tilde{g}, #tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{ 1}^{ 0}, B(#tilde{#chi}_{ 1}^{ 0} #rightarrow #gamma #tilde{G}) = B(#tilde{#chi}_{ 1}^{ 0} #rightarrow Z #tilde{G})=50%}");// #tilde{#chi}_{ 1}^{ 0} #rightarrow #gamma/Z #tilde{G}}");//  NLO+NLL exclusion}");
   modelName="T5ttttZg";}
 else if(modelName.Contains("T5bbbbZg")){
   decayMode->AddText("#bf{pp #rightarrow #tilde{g} #tilde{g}, #tilde{g} #rightarrow b #bar{b} #tilde{#chi}_{ 1}^{ 0}, B(#tilde{#chi}_{ 1}^{ 0} #rightarrow #gamma #tilde{G}) = B(#tilde{#chi}_{ 1}^{ 0} #rightarrow Z #tilde{G})=50%}");//  NLO+NLL exclusion}");
   modelName="T5bbbbZg";}
 else if(modelName.Contains("T6ttZg")){
   decayMode->AddText("#bf{pp #rightarrow #tilde{t} #bar{#tilde{t}}, #tilde{t} #rightarrow t #tilde{#chi}_{ 1}^{ 0}, B(#tilde{#chi}_{ 1}^{ 0} #rightarrow #gamma #t\
ilde{G}) = B(#tilde{#chi}_{ 1}^{ 0} #rightarrow Z #tilde{G})=50%}");// #tilde{#chi}_{ 1}^{ 0} #rightarrow #gamma/Z #tilde{G}}");//  NLO+NLL exclusion}");
   modelName="T6ttZg";}
...
line[3] = new TLine(0.17,0.81,0.22,0.81); line[3]->SetNDC(1);
//line[3] = new TLine(0.17,0.73,0.22,0.73); line[3]->SetNDC(1);
 line[3]->SetLineWidth(gr1d_Median->GetLineWidth()); line[3]->SetLineColor(gr1d_Median->GetLineColor()); line[3]->SetLineStyle(1);//gr1d_Median->GetLineStyle());
...
//legName.DrawLatexNDC(0.237,0.788,"#bf{Observed #pm 1 #sigma_{theory}}");
 if(draw2sigmaLines) legName.DrawLatexNDC(0.237,0.7375,"#bf{Expected (data)}");//#pm 1, 2 #sigma_{experiment}}");
...
// if(draw2sigmaLines) legName.DrawLatexNDC(0.245,0.731,"#bf{Expected #pm 1, 2 #sigma_{experiment}}");
 else  legName.DrawLatexNDC(0.237,0.8,"#bf{Expected (data)}");//#bf{Expected #pm 1 #sigma_{experiment}}");
...
gr2dXsec->GetZaxis()->SetTitleOffset(1.0);
// gr2dXsec->GetZaxis()->SetMoreLogLabels();
// gr2dXsec->GetZaxis()->SetNoExponent();
// gr2dXsec->GetZaxis()->ChangeLabel(1);

gr2dXsec->GetHistogram()->GetZaxis()->SetTitle("95% CL upper limit on cross section (fb)");
gr2dXsec->GetZaxis()->SetTitleSize(0.045);
gr2dXsec->GetZaxis()->SetLabelSize(0.04);
// gr2dXsec->GetZaxis()->SetTitleOffset(0.8);
//gr2dXsec->GetZaxis()->SetTitleOffset(1.0);
