...
void smoothedObsExpLimits_working(TString fName){
  //---------------- set styles----------------
  gStyle->SetOptStat(0);
  gStyle->SetTitle(0);
 ...
 */
 if(modelName.Contains("T5bbbb") || modelName.Contains("T5qqqq")){ xMin = 1500; xMax = 2800; nDivAxis = 510; yMin = 0.; yMax = 3000.; zMin = 1.1e-1, zMax = 40e-1;}
if(modelName.Contains("T5tttt")                              )  { xMin = 1425; xMax = 2450; nDivAxis = 510;}
if(modelName.Contains("T6tt")                                )  { xMin =  825; xMax = 1600; nDivAxis = 510; yMin = 0.; yMax = 2000.;}
if(modelName.Contains("TChi")                                )  { xMin =  200; xMax = 1300; nDivAxis = 510; yMin = 0.; yMax = 800.;}

TPaveText *decayMode = new TPaveText(0.15,0.74, 0.85,0.93,"NDC");
...
//TLegend *legText = new TLegend(0.17,0.79, 0.45,0.82);
  TLegend *legText = new TLegend(0.17,0.74, 0.45,0.84);
TLine *line[9];
...
//  h2_temp->Draw("text");
//  h2_empty->Draw("text");
 TH2D *hist;//=new TH2D("h2temp","h2temp",1000,0,3025,1000,0,3005);
 if(!modelName.Contains("T6ttZg"))hist=new TH2D("h2temp","h2temp",1000,0,3025,1000,0,3005);
 if(modelName.Contains("T6ttZg")) hist=new TH2D("h2temp","h2temp",1000,0,3025,200,0,2005);
hist->SetBinContent(0,0,1);
hist->SetTitle(0);
...
//gr2dXsec->GetXaxis()->SetNdivisions(505);
 ...
 if(modelName.Contains("TChiWZ") || modelName.Contains("TChiWH")) hist->SetTitle(";m(#tilde{#chi}_{ 1}^{ #pm}/#tilde{#chi}_{ 2}^{ 0}) [GeV];m(#tilde{#chi}_{ 1}^{ 0}) [GeV]");
 if(modelName.Contains("TChiWW")) hist->SetTitle(";m_{#tilde{#chi}_{ 1}^{ #pm}} [GeV];m_{#tilde{#chi}_{ 1}^{ 0}} [GeV]");
  hist->SetTitle(0);
 ...
 if(modelName.Contains("T5")) hist->GetXaxis()->SetTitle("m_{#tilde{g}} (GeV)");
 if(modelName.Contains("T6ttZg")) {hist->GetXaxis()->SetTitle("m_{#tilde{t}} (GeV)");}//hist->GetYaxis()->SetNdivisions(10);}
 hist->GetYaxis()->SetTitle("m_{#tilde{#chi}_{ 1}^{ 0}} (GeV)");
 //hist->GetZaxis()->SetTitle("xsec");
//  hist->SetTitle(";m_{#tilde{g}}(GeV);m_{#tilde{#chi}_{ 1}^{ 0}}(GeV)");
//  gr2dXsec->GetZaxis()->SetTitle(";m_{#tilde{g}}(GeV);m_{#tilde{#chi}_{ 1}^{ 0}}(GeV)");


hist->Draw();
//  hist->GetZaxis()->SetTitle("95%");
//  gr2dXsec->SetHistogram(hist);
gr2dXsec->Draw("COLZ same");
gr1d_Median->Draw("C");
gr1d_16pc->Draw("C");
gr1d_84pc->Draw("C");
gr1d_Obs->Draw("C");
...
if(modelName.Contains("T5qqqqHg")){
  //  decayMode->AddText("#bf{pp #rightarrow #tilde{g} #tilde{g}, #tilde{g} #rightarrow q #bar{q} #tilde{#chi}_{ 1}^{ 0}, #tilde{#chi}_{ 1}^{ 0} #rightarrow #gamma/H #tilde{G} (50%) NLO+NLL exclusion}");
...
line[0] = new TLine(0.17,0.83,0.22,0.83); line[0]->SetNDC(1);
...
line[1] = new TLine(0.17,0.837,0.22,0.837); line[1]->SetNDC(1);
...
line[2] = new TLine(0.17,0.823,0.22,0.823); line[2]->SetNDC(1);
...
// if(draw2sigmaLines){
//   line[7] = new TLine(0.17,0.794,0.22,0.794); line[7]->SetNDC(1);
...
line[6] = new TLine(0.17,0.787,0.22,0.787); line[6]->SetNDC(1);
...
line[3] = new TLine(0.17,0.78,0.22,0.78); line[3]->SetNDC(1);
...
line[4] = new TLine(0.17,0.773,0.22,0.773); line[4]->SetNDC(1);
...
legName.DrawLatexNDC(0.237,0.818,"#bf{Observed #pm 1 #sigma_{theory}}");
...
// if(draw2sigmaLines) legName.DrawLatexNDC(0.245,0.731,"#bf{Expected #pm 1, 2 #sigma_{experiment}}");
 legName.DrawLatexNDC(0.237,0.7675,"#bf{Expected #pm 1 #sigma_{experiment}}");
...
textOnTop.DrawLatexNDC(0.15,0.945,"CMS #it{#bf{}}");
//textOnTop.DrawLatexNDC(0.15,0.915,"CMS");
sprintf(name3,"#bf{%0.0f fb^{-1} (13 TeV)}",intLumi);
intLumiE.DrawLatexNDC(0.68,0.945,name3);
 c3->SetCanvasSize(1600,1000);
