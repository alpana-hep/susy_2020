...
int line_width[12] = {3,3,3,3,3,3,2,3,2,3,2,2};
int line_style[12] = {2,2,1,2,1,1,1,2,1,2,1,1};
// /int line_color[n_pl+1] = {kBlack, kRed, kGreen+2, kBlue, kRed};
// /int line_color[n_pl+1] = {kBlack, kRed, kGreen+2, kBlue, kRed};                                                                               
int line_color[9] = {kBlue,kBlack,kBlue,kBlack,kRed,kMagenta, kRed+2,9,kCyan+2};//,45,kMagenta,kGray+1,kRed,kBlue+2,kMagenta,kCyan};
...
void generate_1Dplot(vector<TGraph*> hist, char const *tag_name="",char const *xlabel="",char const *ylabel="", float energy=-1, int rebin=-1,double ymin=0,double ymax=0,int xmin=-1,int xmax=-1, char const *leg_head="",
		     bool normalize=false, bool log_flag=true, bool DoRebin=false, bool save_canvas=true,  vector<string> legend_texts={"nil"}, char const *legend_title="", TString model="", vector<int> linecolorlist={}, vector<int> lineStylelist={},vector<int> lineWidthlist={},int which_plot=-1){  
...
canvas_n1->SetBottomMargin(0.13);
  ...
  legend = new TLegend(0.15,0.77,0.92,0.945);  
  legend->SetTextSize(0.027);
  if(which_plot==1 || which_plot>998 || (which_plot==411 && model.Contains("T5ttttZg")))
 ...
 legend->SetLineColor(kWhite);
  ...
  for(int i =0;i<(int)hist.size(); i ++) {
    ...
    hist.at(i)->SetLineWidth(lineWidthlist[i]);
    hist.at(i)->SetLineStyle(lineStylelist[i]);
...
void overlay_multifile(string pathname, string model, int which_plot)
{
  ...
  vector<int>  lineStylelist = {2,2,1,2,1,1,1,2,1,2,1,1};
  vector<int>  lineWidthlist = {4,4,4,4,4,4,4,4,4,4,4,4};
  ...
  n_files=2;
    ...
    f[0] = new TFile("Excl_out_T5bbbbZg_combine_MET_200_phopt_40.root");//out_T5bbbbZg_2200_10.root"); Excl_out_T5bbbbZg_combine_h_Sbins_LL_MET_200.root
...
if(Model.Contains("T5bbbbZg") && which_plot==199 ){//&& Gluino_m.Contains("2200")){                                                                                 
      legend_title={"#tilde{g} #rightarrow b #bar{b} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};//, m_{#tilde{g}} = 2200 "};         
      legend_texts_v1 = {"Expected (MC)","Expected (data)"};//SP bins(N_{jets},N_{jets}^{b},p_{T}^{miss}) : p_{T}^{#gamma}>40", "SP bins(N_{jets},N_{jets}^{b},p_{T}^{miss}) : p_{T}^{#gamma}>100" };
      f[0] = new TFile("../Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root"); 
      f[1] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v7_MET_200_pred.root");
 ...
 if(Model.Contains("T5qqqqHg") && which_plot==199 ){//&& Gluino_m.Contains("2200")){                                                                                   
      legend_title={"#tilde{g} #rightarrow q #bar{q} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};//, m_{#tilde{g}} = 2200 "};        
      legend_texts_v1 = {"Expected (MC)","Expected (data)"};//SP bins(N_{jets},N_{jets}^{b},p_{T}^{miss}) : p_{T}^{#gamma}>40", "SP bins(N_{jets},N_{jets}^{b},p_{T}^
      f[0] = new TFile("../Excl_out_T5qqqqHg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[1] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_newSbins_v7_MET_200_pred.root");
 ...
 if(Model.Contains("T5ttttZg") && which_plot==199 ){//&& Gluino_m.Contains("2200")){                                                                               
                                                                                                                                                                            legend_title={"#tilde{g} #rightarrow q #bar{q} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};//, m_{#tilde{g}} = 2200 "};        
      legend_texts_v1 = {"Expected (MC)","Expected (data)"};//SP bins(N_{jets},N_{jets}^{b},p_{T}^{miss}) : p_{T}^{#gamma}>40", "SP bins(N_{jets},N_{jets}^{b},p_{T}^
      f[0] = new TFile("../Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[1] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_pred.root");
    ...
    if(Model.Contains("T6ttZg") && which_plot==1 ){
     ...
     if(Model.Contains("T6ttZg") && which_plot==199 ){
      legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} : SP bins(N_{jets},N_{jets}^{b},p_{T}^{miss})"};
      legend_texts_v1 = {"Expected (MC)","Expected (data)"};//"p_{T}^{#gamma}>40", "p_{T}^{#gamma}>100","p_{T}^{#gamma}>40 & BDT score>0", "p_{T}^{#gamma}>100 & BDT score>0" };
      f[0] = new TFile("../Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[1] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_pred.root");
...
linecolorlist = {kBlue, kBlue+1, kYellow+2, kCyan, kViolet,kRed,kMagenta, kRed+2,9, kGreen+2, kYellow+2};
    ...
    if(Model.Contains("T6ttZg") && which_plot==4 ){
      ...
      f[0] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      // f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_v1.root");
      // f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut_v1.root");
      f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut_v1.root");
      linecolorlist = {kBlue, kBlack, kViolet,kViolet,kMagenta, kRed+2,9, kGreen+2, kYellow+2};
      n_files=4;
      ymax={2000};
      xmax={1600};
      ymin={0};
      ...
      png_string = "overlay_v3vsV7STMETNbjet_phoptBins_withMvaCut_T6ttZg_v1";
      lineStylelist = {2,2,1,2,1,2,1,2,1,2,1,1};
  }

    if(Model.Contains("T6ttZg") && which_plot==999 ){
      legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      ...
      f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_v1.root");
      //f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut.root");
      f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      ...
      xmin={800};
       xlabel = {"m_{#tilde{t}} [GeV]"};
      png_string = "overlay_v3vsV7Bins_T6ttZg_final";
      lineStylelist = {2,2,1,1,1,1,1,2,1,2,1,1};
  }

     if(Model.Contains("T6ttZg") && which_plot==6 ){
      legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0","SP bins (N_{jets}^{b},N_{jets},p_{T}^{miss},ST,p_{T}^{#gamma})","SP bins (N_{jets}^{b},N_{jets},p_{T}^{miss},ST,p_{T}^{#gamma}): BDT score>0"};
      f[0] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200_phopt_100.root"); 
      // f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_v1.root");
      // f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut_v1.root");      
      f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut_v1.root");
      //      f[5] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v8_MET_200.root");
      f[4] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v8_MET_200_withMvaCut.root");

      linecolorlist = {kBlue, kBlack,  kViolet,kViolet,kGreen+1, kGreen+2,9, kGreen+2, kYellow+2};
      n_files=5;
      ymax={2000};
      xmax={1600};
      ymin={0};
      xmin={800};
       xlabel = {"m_{#tilde{t}} [GeV]"};
      png_string = "overlay_v3vsV7_v8_STMETNbjet_phoptBins_withMvaCut_T6ttZg";
      lineStylelist = {2,2,1,2,1,1,1,2,1,2,1,1};

  }

     
     if(Model.Contains("T6ttZg") && which_plot==7 ){
      legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40 & BDT score>0", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0","SP bins (N_{jets}^{b},N_{jets},p_{T}^{miss},ST)","SP bins (N_{jets}^{b},N_{jets},p_{T}^{miss},ST): BDT score>0"};
      f[0] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200.root");
      f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut.root");
      ...
      f[5] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut.root");
      f[6] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_v4_MET_200.root");
      f[7] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_v4_MET_200_withMvaCut.root");

      linecolorlist = {kBlue, kBlack, kYellow+2, kYellow+2, kViolet,kViolet,kGreen+1, kGreen+2,9, kGreen+2, kYellow+2};
      n_files=8;
      ymax={2000};
      xmax={1600};
      ymin={0};
      xmin={800};
       xlabel = {"m_{#tilde{t}} [GeV]"};
      png_string = "overlay_v3vsV7_v4_STMETNbjet_withMvaCut_T6ttZg";
  }

          if(Model.Contains("T6ttZg") && which_plot==8 ){
      legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_\
{jets}^{b}):p_{T}^{#gamma}>40 & BDT score>0", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}\
):p_{T}^{#gamma}>40 & BDT score>0","SP bins (N_{jets}^{b},N_{jets},ST,p_{T}^{#gamma})","SP bins (N_{jets}^{b},N_{jets},ST,p_{T}^{#gamma}): BDT score>0"};
      f[0] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200.root");
      f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut.root");
      f[4] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200.root");
      f[5] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut.root");
      f[6] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v6_MET_200.root");
      f[7] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v6_MET_200_withMvaCut.root");
      ...
      xmin={800};
       xlabel = {"m_{#tilde{t}} [GeV]"};
      png_string = "overlay_v3vsV7_STNjetsNbjet_phoptBins_withMvaCut_T6ttZg";
  }
    if(Model.Contains("T6ttZg") && which_plot==9 ){
      legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40 & BDT score>0","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0","SP bins (N_{jets}^{b},N_{jets},ST,p_{T}^{miss})","SP bins (N_{jets}^{b},N_{jets},ST,p_{T}^{#miss}): BDT score>0"};
      f[0] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200.root");
      f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut.root");
      f[4] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200.root");
      f[5] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut.root");
      f[6] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v5_MET_200.root");
      f[7] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v5_MET_200_withMvaCut.root");
      ...
      xmin={800};
       xlabel = {"m_{#tilde{t}} [GeV]"};
      png_string = "overlay_v3vsV7_STNjetsNbjet_METBins_withMvaCut_T6ttZg";
    }



    if(Model.Contains("T6ttZg") && which_plot==411 ){
      legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40 & BDT score>0", "Bins(N_{jets}, N_{jets}^{b}, p_{T}^{miss},ST,p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma})-Merge","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma})-Merge with BDT score>"};
      f[0] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      //f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200_withMvaCut.root");                                                                               
      // f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v3_MET_200.root");
      ...
      f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut_v1.root");
      //f[4] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v8_MET_200.root");
      f[4] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v8_MET_200_withMvaCut.root");
      ...
      xmin={800};
       xlabel = {"m_{#tilde{t}} [GeV]"};
      png_string = "overlay_v3vsV7_BDTimpact_T6ttZg_final";  }

  if(Model.Contains("TChiWG") && which_plot==1 ){
    legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} : SP bins(N_{jets},N_{jets}^{b},p_{T}^{miss})"};
...
a}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0"};
    ...
    xmin={800};
    n_files=5;
    png_string = "STMETNbjet_phoptBins_withMvaCut_TChiWG";
  }
    ...
    if(Model.Contains("T5bbbbZg") && which_plot==41 ){
      ...
      f[0] = new TFile("Excl_out_T5bbbbZg_combine_MET_200_phopt_40.root");
      f[1] = new TFile("Excl_out_T5bbbbZg_combine_MET_200_phopt_100.root");
      // f[2] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v3_MET_200_v1.root");
      // f[3] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut_v1.root");
      f[2] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[3] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut_v1.root");
      linecolorlist = {kBlue, kBlack, kViolet,kViolet,kMagenta, kRed+2,9, kGreen+2, kYellow+2};
      n_files=4;
      ymax={3000};
      xmax={2800};
      ...
      png_string = "overlay_v3vsV7STMETNbjet_phoptBins_withMvaCut_v1";
      lineStylelist = {2,2,1,2,1,2,1,2,1,2,1,1};
  }

    if(Model.Contains("T5bbbbZg") && which_plot==999 ){
      legend_title={"#tilde{g} #rightarrow b #bar{b} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0"};
      f[0] = new TFile("Excl_out_T5bbbbZg_combine_MET_200_phopt_40.root");
      f[1] = new TFile("Excl_out_T5bbbbZg_combine_MET_200_phopt_100.root");
      f[2] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v3_MET_200_v1.root");
      //f[3] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut.root");
      f[3] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      ...
      xmin={1500};
      xlabel = {"m_{#tilde{g}} [GeV]"};
      png_string = "overlay_v3vsV7Bins_T5bbbbZg_final";
       lineStylelist = {2,2,1,1,1,1,1,2,1,2,1,1};
  }

    
    if(Model.Contains("T5bbbbZg") && which_plot==411 ){
      legend_title={"#tilde{g} #rightarrow b #bar{b} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40 & BDT score>0", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma})-Merge","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma})-Merge with BDT score>"};
      f[0] = new TFile("Excl_out_T5bbbbZg_combine_MET_200_phopt_40.root");
      f[1] = new TFile("Excl_out_T5bbbbZg_combine_MET_200_phopt_100.root");
      ...
      f[5] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut.root");
      f[6] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[7] = new TFile("Excl_out_T5bbbbZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut.root");

      linecolorlist = {kBlue, kBlack, kYellow+2, kYellow+2, kViolet,kViolet,kRed, kRed+2,9, kGreen+2, kYellow+2};
      n_files=7;
      ymax={3000};
      xmax={2800};
      ymin={0};
    ...
    if(Model.Contains("T6ttZg") && which_plot==5 ){
      legend_title={"#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} : SP bins(N_{jets},N_{jets}^{b},p_{T}^{miss})"};
      legend_texts_v1 = {"p_{T}^{#gamma}>40", "p_{T}^{#gamma}>100","p_{T}^{#gamma}>40 & BDT score>0", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0","SP bins (N_{jets}^{b},N_{jets},p_{T}^{miss},ST,p_{T}^{#gamma})","SP bins (N_{jets}^{b},N_{jets},p_{T}^{miss},ST,p_{T}^{#gamma}): BDT score>0"};
      f[0] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      f[2] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_MET_200_withMvaCut.root");
      f[3] = new TFile("Excl_out_T6ttZg_combine_h_Sbins_LL_newSbins_v7_MET_200.root");
    ...
    if(Model.Contains("T5qqqqHg") && which_plot==4 ){
      ...
      // f[2] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_newSbins_v3_MET_200_v1.root");
      // f[3] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut_v1.root");
      f[2] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[3] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut_v1.root");
      linecolorlist = {kBlue, kBlack,  kViolet,kViolet,kMagenta, kRed+2,9, kGreen+2, kYellow+2};
      n_files=4;
      ymax={3000};
      xmax={2800};
      ...
      xmin={1500};
       xlabel = {"m_{#tilde{g}} [GeV]"};
      png_string = "overlay_v3vsV7STMETNbjet_phoptBins_withMvaCut_T5qqqqHg_v1";
  }

    if(Model.Contains("T5qqqqHg") && which_plot==999 ){
      legend_title={"#tilde{g} #rightarrow q #bar{q} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0"};
      f[0] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      ...
      f[3] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      ...
      xmin={1500};
       xlabel = {"m_{#tilde{g}} [GeV]"};
      png_string = "overlay_v3vsV7Bins_T5qqqqHg_final";
       lineStylelist = {2,2,1,1,1,1,1,2,1,2,1,1};
  }

     if(Model.Contains("T5qqqqHg") && which_plot==411 ){
      legend_title={"#tilde{g} #rightarrow q #bar{q} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40 & BDT score>0", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma})-Merge","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma})-Merge with BDT score>0"};
      f[0] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      ...
      f[5] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut.root");
      f[6] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_newSbins_v7_MET_200.root");
      f[7] = new TFile("Excl_out_T5qqqqHg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut.root");
      linecolorlist = {kBlue, kBlack, kYellow+2, kYellow+2, kViolet,kViolet,kRed, kRed+2,9, kGreen+2, kYellow+2};
      n_files=8;
      ymax={3000};
      xmax={2800};
      ...
      xmin={1500};
       xlabel = {"m_{#tilde{g}} [GeV]"};
      png_string = "overlay_v3vsV7merge_STMETNbjet_phoptBins_withMvaCut_T5qqqqHg";
  }

       if(Model.Contains("T5ttttZg") && which_plot==6 ){
      legend_title={"#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40 & BDT s\
core>0", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T\
}^{#gamma})-Merge","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma})-Merge with BDT score>0"};
      ...
      f[2] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v7_MET_200.root");
      f[3] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut.root");
      //f[6] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v8_MET_200.root");
      f[4] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v8_MET_200_withMvaCut.root");
      ...
      xmin={1400};
      xlabel = {"m_{#tilde{g}} [GeV]"};
      png_string = "overlay_v3vsV7vsV8_STMETNbjet_phoptBins_withMvaCut";
  }

  if(Model.Contains("T5ttttZg") && which_plot==411 ){
      legend_title={"#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40 & BDT score>0","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma})-Merge","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma})-Merge with BDT score>0"};
      f[0] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      ...
      f[2] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[3] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut_v1.root");

      linecolorlist = {kBlue, kBlack, kViolet,kViolet,kYellow+2, kYellow+2, kViolet,kViolet,kRed, kRed+2,9, kGreen+2, kYellow+2};
      n_files=4;
      ymax={3000};
      xmax={2800};
      ymin={0};
      xmin={1400};
      xlabel = {"m_{#tilde{g}} [GeV]"};
      png_string = "overlay_impactBDT_v7";
  }
    ...
    if(Model.Contains("T5ttttZg") && which_plot==4 ){
      ...
      f[0] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      f[2] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_v1.root");
      f[3] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut_v1.root");
      f[4] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      f[5] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut_v1.root");
...
if(Model.Contains("T5ttttZg") && which_plot==999 ){
      legend_title={"#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G}"};
      legend_texts_v1 = {"SP bins : p_{T}^{#gamma}>40","SP bins : p_{T}^{#gamma}>100","Bins(p_{T}^{miss},ST,N_{jets}^{b}):p_{T}^{#gamma}>40", "Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}):p_{T}^{#gamma}>40","Bins(p_{T}^{miss},ST,N_{jets}^{b},p_{T}^{#gamma}\
):p_{T}^{#gamma}>40 & BDT score>0"};
      f[0] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_MET_200.root");
      f[1] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_MET_200_phopt_100.root");
      f[2] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_v1.root");
      //f[3] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v3_MET_200_withMvaCut.root");
      f[3] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_v1.root");
      //f[5] = new TFile("Excl_out_T5ttttZg_combine_h_Sbins_LL_newSbins_v7_MET_200_withMvaCut.root");
      linecolorlist = {kBlue, kBlack, kYellow+2,kViolet,kViolet,kMagenta, kRed+2,9, kGreen+2, kYellow+2};
      n_files=4;
      ymax={3000};
      ...
      xmin={1400};
      xlabel = {"m_{#tilde{g}} [GeV]"};
      png_string = "overlay_v3vsV7Bins_T5ttttZg_final";
       lineStylelist = {2,2,1,1,1,1,1,2,1,2,1,1};
  }
 ...
 for(int i_cut=0; i_cut<n_cut;i_cut++){
   ...
   for(int i_var=0; i_var<n_var;i_var++)
	  ...
	  generate_1Dplot(hist_list_Njets,full_path,xlabel[i_var].c_str(),ylabel[i_var].c_str(),energy,rebin[i_var],ymin[i_var],ymax[i_var],xmin[i_var],xmax[i_var],legend_texts[i_cut].c_str(),false,true,false,true,legend_texts_v1,legend_title[0].c_str(), Model, linecolorlist, lineStylelist,lineWidthlist,which_plot);
