...
TString legend_text[4] = {"(0#mu,0e) SR","(1#mu,0e) CR","pMSSM_MCMC_106_19786","pMSSM_MCMC_473_54451"};//{"No cuts","skimmed","lep-veto","isotrk-veto","Dphi-cut","MET>250","ST>300","Pho-pt>100"};
...
vector<int> col={kMagenta+2, kGray+2,9,7,45,kViolet,kGreen-2,kYellow+1,kGray+2,kMagenta,kBlue+2,kMagenta,kCyan};
...
hs_var->GetXaxis()->SetTitleSize(00.05);
  ...
  gPad->Modified();
    ...
    hist_ratio->GetYaxis()->SetRangeUser(-3,3);
...
void Stacked_LL_varRatio(string pathname, bool which_Lept)
{
  ...
  if(which_Lept)
    baseline1={"Nocut","bin_Elec_SR","bin_Elec_CR"};//
  else
    baseline1 = {"Nocut","bin_Mu_SR","bin_Mu_CR"};
  ...
  for(int i_file=0; i_file<8;i_file++)
      ...
      for(int i_cut=1; i_cut<3;i_cut++)
	{
      ...
      hNjets_ratio->GetXaxis()->SetTitle("N_{jets}");
      hNjets_ratio->GetYaxis()->SetTitle(" ");
      hNjets_ratio->SetTitle("");
      //      hNjets_ratio->
      hNjets_ratio->Divide(hist_list_Njets.at(1));
      sprintf(full_path,"%s/Overlay_NJets_%s_",pathname.c_str(),filetag[i_file]);
      generate_1Dplot(hist_list_Njets,hNjets_ratio,full_path,energy,16,0,leg_head,false,true,false,true,filetag[i_file]);
